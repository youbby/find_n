...
### — 생산 간격 평탄화를 위한 이동평균 윈도우(N) 선정 사례 연구 (개정판 v5)

---

## 변경 이력 (Change Log)

본 문서는 네 차례의 적대적 피어리뷰를 거쳤다(v4까지는 "두 차례"로 오기되어 있었다 —
이번 개정에서 정정). 각 라운드에서 **직전 판본의 핵심 논리는 그대로 유지**하고 지적된
부분만 국소적으로 교체·보강하였다.
...
- **v4 → v5**: 4차 리뷰(Critical 4, Major 5, Minor 4) — **결손 실험의 이상 현상(C8) 규명 최우선** ← 본 개정
...
| 8.4 | **표 재구성** | M17 | 권고표에 용도(목적1/목적2) 열 추가 |
...
> **[개정 v5 — FFT의 P=3 vs P=5 성능 차이 진단 (Minor)]** 위 표에서 FFT는 같은 순차 stage 구조인데 P=3 100%와 P=5 42.2%로 크게 갈린다. 실패 사례를 직접 확인하면 원인이 드러난다 — P=5가 실패할 때 FFT는 예외 없이 **정확히 2**를 반환한다(고대비/저대비, 정상/드리프트/중간시프트 전 조건에서 동일). 이는 펄스형 신호의 듀티비(duty cycle) 문제로 설명된다. 순차 stage 패턴은 P개 위상 중 1개만 크고 나머지 (P−1)개는 균일한 "좁은 펄스" 신호다 — 듀티비는 1/P다. 구형파 펄스열의 n차 조화파 세기는 sinc(n/P)에 비례하므로, **듀티비가 작을수록(P가 클수록) 에너지가 기본 주파수(period=P)보다 높은 조화파 쪽으로 밀린다.** P=3(듀티비 1/3)은 기본 주파수가 여전히 우세하지만, P=5(듀티비 1/5, 더 좁은 펄스)는 2차 조화파(주파수 2/P, 정수 주기로 반올림하면 2)가 기본 주파수를 앞지르는 경우가 절반 이상이다. 즉 **P=5의 FFT 실패도 다른 실패들과 마찬가지로 옥타브(고조파) 오류의 한 변종**이며, 지표의 무작위적 결함이 아니다.
...
**결과 — 순위가 다시 한번 뒤집힌다** [로그 G:L169-174]
...
**[개정 v3 — 검정 방식의 정정 (C3)]** v2는 여기서 "두 Wilson 신뢰구간이 겹치지 않으므로 유의하다"고 주장했다. 그러나 8.2.2절에 적었듯 6개 방법은 **동일한 540개 데이터셋에서 평가된 대응표본**이므로, 독립 이항을 가정하는 Wilson 구간 겹침 판정은 이 자리에 적용할 수 없다(겹침만으로 유의성을 부정하면 검정력을 잃고, 겹치지 않음만으로 유의를 주장하면 대응 구조를 무시한다). 15개 방법 쌍 전부에 **McNemar 정확검정**을 적용하고 **Holm-Bonferroni**로 다중비교를 보정하였으며, 차이의 구간은 시행 단위 **대응표본 부트스트랩**으로 구했다[로그 C:L266-294].
...
4. **우세 폭은 주기에 따라 균일하지 않지만, 그 불균일 자체가 패턴 난이도와 교란되어 있었다(M6).** v2는 "P=5에서 기하 100%, P=6에서 25%"를 주기 효과로 읽었으나, 대비 지표 C가 P=5 low에서 0.087, P=4 high에서 1.316으로 15배 차이났다(v3 정의). 대비를 맞춰 재실험하면 **기하 2단계의 열세는 오히려 커지고(+26.3%p, Holm p = 3.54×10⁻¹⁴), P=6 성공률은 0%로 떨어진다**(8.2.3절). 따라서 P=6 취약성은 난이도 탓이 아니라 **방법 자체의 성질**이다.
...
7. **산발적 검출 누락(결손)이 있으면 순위가 다시 한번, 이번엔 크게 뒤집힌다 (v4 신규, 8.2.5절).** 이 설비에서 오검출(삽입)은 실재하지 않지만 검출 누락은 현실적 위협이다. 5% 미만의 산발적 결손만으로 클린 데이터 1위인 직접위상(일반화)은 39.7%로 추락하고, **McNemar 대응표본 검정으로 확인하면 기하 2단계는 ACF(전역최대)와 통계적 동률(공동 1위)이 되며 직접위상(일반화)을 포함한 나머지 4개 방법을 전부 유의하게 이긴다**(n=1,320, Holm 보정). 원인은 알고리즘이 아니라 데이터 구조다 — 결손이 만드는 인덱스 밀림이 후보 주기 N의 약수와 우연히 일치할 확률(1/N)이 작은 N일수록 높으므로, 산발적 결손은 검출을 참 주기의 작은 약수 쪽으로 체계적으로 편향시킨다. 저대비 조건과 결손이 겹치면 **6개 방법 전부가 예외 없이 0%로 무너지는** 시나리오도 존재한다.
...
| 용도 / 데이터 상태 | 권고 | 근거 |
|---|---|---|
| 1매당 시간 산출 (목적 1), **클린 로그** | **직접위상(일반화)** 92.1% | 8.2.4절, 설비 유형 무관하게 90%대 |
| 1매당 시간 산출 (목적 1), **검출 누락 위험 있음** | **ACF(전역최대)** 62.0% — 기하 2단계(60.3%)와 McNemar로 통계적 동률(Holm p=0.147)이나 구현이 압도적으로 싸다(~10줄) | 8.2.5절 |
...
두 규정 모두에서 **기하 2단계가 단독 1위인 경우는 없다.** 클린 로그에서는 직접위상(일반화)에 유의하게 밀리고(68.4% vs 92.1%, Holm p=1.30×10⁻³⁹), 결손 위험 조건에서는 ACF(전역최대)와 McNemar 대응표본 검정으로 **통계적 동률**이다(차이 +1.7%p, Holm p=0.147)[8.2.5절]. 동률인 두 방법 사이에서는 구현 비용이 결정적이다 — ACF(전역최대)는 ~10줄, 기하 2단계는 훨씬 많은 코드와 여러 단계의 시행착오를 거쳐야 하는 파이프라인이므로, 성능이 같다면 더 싼 쪽을 쓰지 않을 이유가 없다. **따라서 "표준 기법에 자리를 내주는 것이 합리적"이라는 결론은 데이터 품질 조건이 바뀌어도 유지되지만, 그 표준 기법이 무엇인지는 조건에 따라 직접위상(일반화)→ACF(전역최대)로 달라진다.**

**기하학적 방법의 위치**: 클린 물리 기반 세트에서 6개 중 3위(68.4%)로, ACF(첫국소최대)(+13.3%p)와 직접위상(일반화)(+23.7%p)에 **유의하게 밀리고**, ACF(전역최대)·직접위상(2배)·FFT는 **유의하게 이긴다.** 결손 스트레스 조건에서는 위치가 뒤바뀐다 — ACF(전역최대)와는 McNemar로 통계적 동률(공동 1위)이 되고, 클린 조건에서 자신을 이겼던 ACF(첫국소최대)와 직접위상(일반화)을 이번엔 **유의하게 이긴다**(각각 Holm p=8.48×10⁻²³, 2.28×10⁻⁴⁴). 다만 알림 적합성은 클린 조건에서 여전히 더 나쁘고(오경보 37.8%, 미탐 10.8%), 결손 조건에서의 알림 적합성은 측정하지 않았다(9절 남은 과제). 구현 비용이 두 데이터 품질 조건 모두에서 가장 큰 것을 감안하면, 동률인 상대(ACF 전역최대)가 훨씬 싸게 같은 성능을 내므로 **어느 조건에서도 이 복잡도를 정당화할 근거는 없다** — 다만 v3까지의 "표준 기법에 밀린다"는 뭉뚱그린 서술은 **"클린 데이터에서는 두 방법에 밀리고 세 방법을 이기며, 결손 스트레스 조건에서는 가장 싼 표준 기법과 통계적으로 동률이면서 나머지 넷을 유의하게 이긴다"**로 정정된다.
...
같은 물리 기반 패턴에 산발적 검출 누락(rate<5%)을 섞으면 순위가 다시 뒤집힌다. 올바른 비교 기준선(정상 조건 rate=0, 330회)으로 확인하면 직접위상(일반화)은 77.9%→39.7%(−38.2%p)로 붕괴하고, 기하 2단계는 62.7%→60.3%(−2.4%p)로 거의 그대로다. McNemar 검정(n=1,320)으로 기하 2단계는 ACF(전역최대)와 통계적 동률(공동 1위)이 되며 나머지 넷을 유의하게 이긴다. 다만 **결손이 성능을 개선하는 사례가 11패턴 중 8개에서 관측**되었다 — clean 데이터에서 참 주기의 배수들이 median H·ACF 값에서 사실상 tie를 이루는데, 결손이 그 tie를 깨면서 가장 작은 배수만 남기 때문이며, 이는 8.2.5절 본편의 "결손이 작은 약수 쪽으로 편향시킨다"는 원리의 반대쪽 얼굴이다(코드 버그 아님, C8에서 규명). 동률 상대(ACF 전역최대)가 구현 비용은 압도적으로 싸므로, 이 결과도 기하학적 방법의 채택 근거가 되지는 않는다.
...
또한 개정 과정에서 여섯 가지를 새로 규명했다. 첫째, **v1이 근거로 삼은 직접 위상비교 구현은 2의 거듭제곱 주기만 반환할 수 있어**, v1의 "6/6 성공" 결론은 P=4에 한정된 것이었다(8.1절). 둘째, **ACF의 P=6 실패는 지표가 아니라 판정 규칙 때문**이며, 규칙을 바꾸면 성능 순위가 뒤집힌다(8.3절). 셋째, **통제 세트에서 기하 2단계가 P=6에서 0%인 것도 같은 원인**이다 — median H는 참 주기 N=6(0.588)을 N=1(0.988)보다 정확히 낮게 가리키는데, "오름차순 첫 정합점" 규칙이 N=1을 먼저 채택한다(8.2.3절 4항). 넷째, **홀수 주기 P=5가 쉬웠던 것은 방법의 우수성이 아니라 짝수 배수 혼동원의 부재** 때문이다(8.4절). 다섯째, **같은 "약수로의 편향" 원리가 산발적 결손에도 적용된다** — 나눠떨어질 작은 약수가 없는 홀수(소수) 주기는 결손에도 강하다(8.2.5절). 여섯째, **그 편향 원리는 방향을 바꿔 "결손이 성능을 개선하는" 사례도 설명한다** — clean 데이터에서 참 주기의 배수들이 통계적으로 tie를 이루는 경우, 결손이 그 tie를 깨면서 가장 작은 배수만 남긴다(8.2.5절, C8). 여섯 가지 모두 같은 곳을 가리킨다 — **이 문제에서 비교의 단위는 "지표"가 아니라 "지표 + 판정 규칙 + 채점 기준 + 데이터 품질"이라는 평가 체계 전체**다.
...
| v5 | `__V5_COMMIT_HASH__` | `verification_tooth_v2.py`, `verification_tooth_v2_log.txt`, 본 문서(citation 정정 포함) |
...
git checkout __V5_COMMIT_HASH__                                                # v5 판본
...
## Appendix D. 리뷰 대응 체크리스트 *(v5에서 네 라운드 통합)*
...
| **M17** | 권고표 용도 열 + 상충 설명 | ✅ 완료 | 8.4절 표 아래 각주 |
