...
### — 생산 간격 평탄화를 위한 이동평균 윈도우(N) 선정 사례 연구 (개정판 v3)
...
- **v2 → v3**: 2차 리뷰(Critical 2, Major 5, Minor 5) 반영 ← 본 개정
...
이 최종 방법을 참 주기 P=4/6/5, 고대비/저대비, 정상/사인드리프트/중간시프트의 **18개 시나리오 × 30회 반복(총 540회 시행)**으로 표준 기법과 비교하였다. 6개 방법이 동일한 540개 데이터셋에서 평가된 대응표본이므로 **McNemar 정확검정 + Holm 보정**으로 비교하였고, 성공은 **반환값이 참 주기와 정확히 일치할 때(strict)**로 정의하되 배수를 인정하는 기준의 결과를 병기하였다. 성공률(strict)은 직접 위상비교(일반화) 91.9%, ACF(첫국소최대) 76.1%, 기하학적 2단계 64.3%, FFT 0%였다. 즉 **기하학적 방법은 구현 복잡도가 가장 크면서 검출 성공률은 유의하게 낮다**(vs ACF +11.9%p, Holm p = 3.9×10⁻⁸). 다만 그 열세는 모든 표준 기법에 대한 것이 아니며(ACF 전역최대와는 유의하지 않음), **채점 기준을 바꾸면 순위가 바뀌고**, 성능을 좌우하는 것이 지표뿐 아니라 **판정 규칙과 채점 기준**이라는 점도 함께 확인되었다. 주기별 성능 차이는 패턴 난이도와 교란되어 있었으며, 대비를 정량화해 통제한 재실험에서는 기하학적 방법의 열세가 오히려 커졌다. 기하학적 접근은 스케일 종속성·과잉 정규화·강건성-검출력 트레이드오프라는 함정을 발견하고 해결하는 **탐구 과정으로서 가치가 있었으나, 실전 배포용 알고리즘으로는 표준 기법에 자리를 내주는 것이 합리적**이라는 것이 본 논문의 결론이다. 다만 **이 결론은 전적으로 합성 데이터에 근거한다** — 실측 생산 로그는 사내 비공개 자료여서 수록·검증할 수 없었으며, 이는 본 연구의 가장 큰 외적 타당성 한계다(7.3절, 검증 절차는 7.3.1절에 명세).
...
median H는 N에 무관하게 상수로 유지되었으나, **median κ는 N이 커질수록 급격히 감소**하였다(N=4 대비 (N/4)²로 나누면 대략 일치 — κ가 1/N²에 비례). 각 항의 스케일을 직접 분해하면 원인이 드러난다(seed=42, median 기준)[로그 L52-61]:
...
**strict를 주 기준으로 채택하는 이유**는 §1의 목적에 있다. 이 논문의 N은 순수한 주기 지식이 아니라 **이동평균 윈도우로 쓸 값**이다. 윈도우를 2P나 3P로 잡아도 평탄화 자체는 되지만, 필요 이상으로 긴 윈도우는 응답 지연을 키우고 유효 표본 수를 줄여 실무 목적을 훼손한다. 즉 **최소 주기를 찾는 것이 과제의 정의 자체**이므로, 배수를 정답으로 인정하면 과제가 느슨해진다.
...
3주기 × 3조건 × 30회 = **270회 시행** 결과[로그 L456-467]:
...
1. **기하학적 방법의 열세는 대비를 맞춰도 사라지지 않는다 — 오히려 커진다.** ACF(첫국소최대)와의 격차가 원래 세트의 +11.9%p에서 **+26.3%p**로 벌어졌고, McNemar Holm p = 3.54×10⁻¹⁴로 여전히 유의하다[로그 L526]. **v2의 핵심 결론은 난이도 교란을 제거한 뒤에도 유지된다.**
2. **"P=5가 쉽다"는 v2의 관찰은 상당 부분 난이도 효과였다.** 대비를 맞추자 기하 2단계의 P=6 성공률이 25.0% → **0.0%**로 더 떨어져, 주기별 격차가 좁혀지기는커녕 극단화되었다. 즉 이 방법의 P=6 취약성은 패턴 난이도 탓이 아니라 **방법 자체의 성질**이다.
3. **FFT가 0% → 100%로 뒤집힌다.** 이는 8.3절에서 진단한 대로 FFT의 0%가 지표의 무능이 아니라 **"큰 값/작은 값 교대" 패턴이 만든 2차 조화파(옥타브 오류)의 결과**였음을 결정적으로 확인해 준다. 교대 구조가 사라지자 단순 최대 피크 규칙만으로 전 주기를 맞혔다. **따라서 v2가 FFT를 최종 권고에서 제외한 판단은 옳았고, 그 이유는 v3에서 실증되었다.**
...
2. **다만 모든 표준 기법에 밀리는 것은 아니다.** 기하 2단계 vs ACF(전역최대)는 차이 +5.9%p이지만 Holm 보정 후 **유의하지 않다**(p = 0.071). v2가 Wilson 구간으로 뭉뚱그렸던 "표준 기법이 우위"라는 서술은, 정확히는 **"ACF(첫국소최대)와 직접위상(일반화)에 대해서만 유의하게 열세"**로 좁혀진다.
...
6. **FFT의 0%는 지표의 무능이 아니라 패턴 구조의 산물임이 확인되었다.** 대비를 통제한 세트(큰 값/작은 값 교대 구조 없음)에서 FFT는 **100%**로 뒤집힌다(8.2.3절). v2가 FFT를 "배음 보정 없이 평가되었으므로 최종 권고에서 제외"한 판단은 옳았고, v3에서 그 이유가 실증되었다. **본 논문의 실험 설계로는 FFT의 실력을 평가할 수 없다.**
...
**따라서 개정된 권고는**: 실전 배포용으로는 **다중비교를 보정한 일반화 직접 위상비교**가 가장 견고하고, ACF는 구현 비용 대비 성능이 우수하나 판정 규칙 선택에 민감하다(첫국소최대와 전역최대가 서로 다른 주기에서 강하다). 기하학적 방법은 성능·비용 양면에서 이들에 앞설 근거가 없다 — 단 그 열세가 유의한 상대는 ACF(첫국소최대)와 직접위상(일반화)이며, ACF(전역최대)에 대해서는 유의하지 않다(위 2항).
...
또한 개정 과정에서 세 가지를 새로 규명했다. 첫째, **v1이 근거로 삼은 직접 위상비교 구현은 2의 거듭제곱 주기만 반환할 수 있어**, v1의 "6/6 성공" 결론은 P=4에 한정된 것이었다(8.1절). 둘째, **ACF의 P=6 실패는 지표가 아니라 판정 규칙 때문**이며, 규칙을 바꾸면 성능 순위가 뒤집힌다(8.3절). 셋째, **홀수 주기 P=5가 쉬웠던 것은 방법의 우수성이 아니라 짝수 배수 혼동원의 부재** 때문이다(8.4절). 세 가지 모두 같은 곳을 가리킨다 — **이 문제에서 비교의 단위는 "지표"가 아니라 "지표 + 판정 규칙 + 채점 기준"이라는 평가 체계 전체**다.
...
**남은 과제**는 다음과 같다. (a) 7.3.1절의 절차에 따라 사내에서 실측 검증을 수행하고, 집계값 공개 또는 대리 데이터 공개 중 하나로 7.3절을 채우는 것(현재 미검증 상태), (b) FFT에 배음 보정(예: 조화적 곱 스펙트럼)을 적용해 교대 패턴에서도 공정하게 재평가하는 것 — 8.2.3절이 FFT의 잠재력을 보인 만큼 우선순위가 높아졌다, (c) 판정 규칙 자체를 데이터에서 학습하거나 주기 후보별로 적응시키는 방법의 탐색, (d) Σ⁻ₙ(n) ≈ N·d 근사의 이론적 오차 상한 유도, (e) 기하 2단계가 대비 통제 세트의 P=6에서 0%가 되는 원인 규명 — 8.3절의 반환값 분포 진단을 이 세트에도 적용해야 한다.
...
**저장소**: https://github.com/youbby/find_n — 본 판본에 대응하는 커밋 해시는
아래 "재현 방법"의 명령으로 확인할 수 있다.
...
cd find_n
git log -1 --format=%H          # 본 논문이 참조하는 커밋 해시
...
## Appendix D. 리뷰 대응 체크리스트 *(v3에서 두 라운드 통합)*
