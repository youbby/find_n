...
**설계**: 8.2.4절의 물리 기반 11개 패턴 전체(정상 조건)에 대해, 각 tact 위치를 독립적으로 확률 rate로 판정해 결손시킨다(고정 개수가 아니라 확률로 판정하므로 실제 발생 개수는 이항분포를 따르는 확률변수다). rate = 0%, 0.5%, 1%, 2%, 4%(모두 5% 미만), 각 조합 30회 반복 — 11패턴 × 5rate × 30회 = **1,650회 시행**[로그 L52-106].

**메커니즘**: 결손 하나가 만드는 인덱스 밀림(shift)은 후보 주기 N에 대해 shift mod N = 0일 때만 "안 보이는" 오차가 된다. 이 확률은 1/N이므로 **작은 N일수록 원천적으로 결손에 안전**하다 — 이는 특정 알고리즘의 결함이 아니라 데이터의 잔차 구조 문제이므로 ACF·직접 위상비교·기하학적 방법을 가리지 않고 똑같이 작용한다. 실제로 median H를 비교하면 이 비대칭이 드러난다(seed=6000, rate=0.5%, 결손 93개)[로그 L13-31]:
...
**8.2.4절의 클린 데이터 순위(직접위상(일반화) 92.1% 1위, 기하 2단계 68.4% 3위)가 거의 정반대로 뒤집힌다.** 산발적 결손 하에서는 ACF(전역최대)와 기하 2단계가 60%대로 올라오고, 클린 데이터의 최강자였던 직접위상(일반화)은 39.7%로 하위권에 머문다. 원인은 8.2.1절에서 이미 지적한 구조 그대로다 — 직접위상(일반화)은 **전역 인덱스 스트라이드**(`y[i::M]`)에 의존하므로, 결손 한 번으로 인덱스-위상 대응이 전역적으로 밀리면 스트라이드 그룹 자체가 통째로 오염된다.

**[McNemar 검정 — Wilson CI 겹침이 아니라 대응표본으로 확인]** 6개 방법은 rate>0의 모든 시행에서 **동일한 데이터셋**을 평가했으므로, C3·C5와 같은 이유로 marginal Wilson CI의 겹침이 아니라 대응표본 검정으로 순위를 확정해야 한다. 기하 2단계를 기준으로 나머지 5개 방법과 McNemar 정확검정 + Holm 보정을 수행하였다(n=1,320)[로그 L178-189]:
...
**저대비와 결손이 겹치면 방법과 무관하게 완전히 무너진다.** P=4 2매세트-low, P=6 2매세트-low는 rate≥0.5%에서, P=4 카세트지연-low(6.2절의 조기경보 시나리오)는 rate≥1.0%에서 **6개 방법 전부가 예외 없이 0%**로 붕괴한다[로그 L73-76, L84-86, L103-106]. 단 하나의 예외가 있다 — **P=4 카세트지연-low, rate=0.5%(결손 약 78개)에서는 기하 2단계 홀로 100%를 유지하고 나머지 다섯 방법은 이미 0%다**[로그 L83]. 단일 시나리오·단일 rate의 결과라 일반화하기는 이르지만, 국소 이웃(n-N, n, n+N)만 보는 기하학적 지표가 극도로 좁은 조기경보 구간에서 유일하게 버틴 사례로 기록해 둔다.
...
> **[한계]** 본 절은 §1.2 목적 1(1매당 시간 산출) 관점의 성공률만 측정하였다. 목적 2(설비 이상 알림, 오경보/미탐)를 결손 조건에서 재평가하는 것은 남은 과제다(9절).
...
**[개정 v4] 산발적 검출 누락(결손)은 클린 데이터 순위를 다시 한번 뒤집는다.** 이 설비에서 오검출(삽입)은 실재하지 않지만 검출 누락은 현실적이다. 5% 미만의 산발적 결손만으로 클린 데이터 1위인 직접위상(일반화)은 39.7%로 추락한다. **McNemar 대응표본 검정으로 확인하면(n=1,320, Holm 보정), 기하 2단계는 ACF(전역최대)와 통계적 동률(공동 1위, 차이 +1.7%p, Holm p=0.147)이 되고, 직접위상(일반화)을 포함한 나머지 4개 방법을 전부 유의하게 이긴다.** 원인은 알고리즘이 아니라 데이터 구조다 — 결손이 만드는 인덱스 밀림이 후보 주기 N의 약수와 우연히 일치할 확률(1/N)이 작은 N일수록 높으므로, 산발적 결손은 검출을 참 주기의 작은 약수 쪽으로 체계적으로 편향시킨다(8.2.5절). 다만 두 공동 1위 방법 중에서는 구현 비용이 압도적으로 싼 ACF(전역최대)를 택하지 않을 이유가 없으므로, 이 발견이 기하학적 방법의 채택 근거가 되지는 않는다. 저대비 조건과 결손이 겹치면 **6개 방법 전부가 예외 없이 0%로 무너지는** 시나리오도 존재한다.

또한 개정 과정에서 다섯 가지를 새로 규명했다. 첫째, **v1이 근거로 삼은 직접 위상비교 구현은 2의 거듭제곱 주기만 반환할 수 있어**, v1의 "6/6 성공" 결론은 P=4에 한정된 것이었다(8.1절). 둘째, **ACF의 P=6 실패는 지표가 아니라 판정 규칙 때문**이며, 규칙을 바꾸면 성능 순위가 뒤집힌다(8.3절). 셋째, **통제 세트에서 기하 2단계가 P=6에서 0%인 것도 같은 원인**이다 — median H는 참 주기 N=6(0.588)을 N=1(0.988)보다 정확히 낮게 가리키는데, "오름차순 첫 정합점" 규칙이 N=1을 먼저 채택한다(8.2.3절 4항). 넷째, **홀수 주기 P=5가 쉬웠던 것은 방법의 우수성이 아니라 짝수 배수 혼동원의 부재** 때문이다(8.4절). 다섯째, **같은 "약수로의 편향" 원리가 산발적 결손에도 적용된다** — 나눠떨어질 작은 약수가 없는 홀수(소수) 주기는 결손에도 강하다(8.2.5절). 다섯 가지 모두 같은 곳을 가리킨다 — **이 문제에서 비교의 단위는 "지표"가 아니라 "지표 + 판정 규칙 + 채점 기준 + 데이터 품질"이라는 평가 체계 전체**다.
...
**남은 과제**는 다음과 같다. (a) 7.3.1절의 절차에 따라 사내에서 실측 검증을 수행하고, 집계값 공개 또는 대리 데이터 공개 중 하나로 7.3절을 채우는 것(현재 미검증 상태), (b) FFT에 배음 보정(예: 조화적 곱 스펙트럼)을 적용해 교대 패턴에서도 공정하게 재평가하는 것 — 8.2.3절이 FFT의 잠재력을 보인 만큼 우선순위가 높아졌다, (c) 판정 규칙 자체를 데이터에서 학습하거나 주기 후보별로 적응시키는 방법의 탐색, (d) Σ⁻ₙ(n) ≈ N·d 근사의 이론적 오차 상한 유도, (e) 물리 기반 세트에서도 기하 2단계·ACF(첫국소최대)가 P=6에서만 25~30%로 무너지는 원인 규명 — 통제 세트의 P=6 0%는 v4에서 판정 규칙 문제로 규명되었으나(8.2.3절 4항), 교대형 P=6의 실패는 아직 미규명이다, (f) 산발적 결손 조건에서 설비 이상 알림(§1.2 목적 2, 오경보/미탐)을 재평가하는 것 — 8.2.5절은 시간 산출(목적 1) 성능만 측정했다, (g) P=4 카세트지연-low, rate=0.5%에서 기하 2단계만 홀로 살아남은 단일 사례(8.2.5절)가 재현 가능한 패턴인지, 더 넓은 rate·패턴 조합으로 확인하는 것.
...
스크립트 `verification_tooth.py`로 산출하였다.
...
| 대응표본 검정 | Wilson CI 겹침이 아니라 McNemar+Holm으로 확정 | ✅ 완료 (n=1,320) | 8.2.5절, 로그 L178-189 |
