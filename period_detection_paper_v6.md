...
### — 생산 간격 평탄화를 위한 이동평균 윈도우(N) 선정 사례 연구 (개정판 v6)

---

## 변경 이력 (Change Log)

본 문서는 다섯 차례의 적대적 피어리뷰를 거쳤다(v4까지는 "두 차례"로 오기되어 있었다).
각 라운드에서 **직전 판본의 핵심 논리는 그대로 유지**하고 지적된 부분만 국소적으로
교체·보강하였다.

- **v1 → v2**: 1차 리뷰(Critical 2, Major 3, Minor 8) 반영
- **v2 → v3**: 2차 리뷰(Critical 2, Major 5, Minor 5) 반영
- **v3 → v4**: 3차 리뷰(Critical 3, Major 4, Minor 4) + **설비 물리 정보 반영**
- **v4 → v5**: 4차 리뷰(Critical 4, Major 5, Minor 4) — **결손 실험의 이상 현상(C8) 규명 최우선**
- **v5 → v6**: 5차 리뷰(Critical 4, Major 3, Minor 4) — **C8/C9/C10/C11의 전파 누락 정정 + rate별 층화 분석(C15)** ← 본 개정

### v5 → v6 변경 이력 *(이번 개정)*

5차 리뷰는 두 갈래였다. **(가) 이미 고친 내용이 다른 절에 전파되지 않은 문제**(C12~C14,
M18, M20) — v4에서 C10·C11이 §9에만 적용되고 8.3·8.4절이 뒤처졌던 것과 같은 유형의
누락이 v5에서도 반복되었다(C9가 8.2.5절에만 적용되고 8.4절 권고표는 그대로였다). **(나)
C8 규명이 필수로 만든 층화 분석**(C15, M19) — rate>0 전체 집계는 서로 반대 방향인 두
메커니즘(약수 편향/배수 tie 해소)을 뭉친 값이므로, rate별로 나눠 봐야 한다.

| 절 | 조치 | 대응 | 요지 |
|---|---|---|---|
| 8.4 | **첫 항목 교체 + 소절 재구성** | C12 | 대표 결과를 물리 기반 세트(990회, 68.4%)로, 540회 McNemar는 "방법론적 시행착오의 기록"으로 이동 |
| 8.3 | **문장 교체** | C13 | "FFT 최종 권고에서 제외" → 8.4절 6항의 조건부 복귀 결론으로 통일 |
| 8.4 | **4항 교체** | C14 | "방법 자체의 성질" → 8.2.3절 4항의 "통제 세트 인공물" 결론으로 정정, 물리 기반 세트 P=6은 별개 미규명으로 분리 |
| 8.2.5 | **표 신설 + 권고표 경고 추가** | C15 | rate별 층화 성공률·순위·McNemar 표, rate=0.5% 기하2단계 단독 1위, ACF(전역최대) 단조 개선 명시 |
| 8.4 | **권고표 수치 병기** | M18 | 클린 행에 77.9%(rate=0, 330회) 병기, 92.1%(990회)와 구분 |
| 8.2.5 | **한계 문단 추가** | M19 | rate 동일 가중이 임의적 가정임을 명시 |
| Change Log, Appendix D | **표기 정정** | M20 | M17을 "표 아래 스코프 설명 추가"로 재표기(열 추가가 아니었음) |
| 8.2.5 | **소제목 수정** | Minor | "990회 대비 뒤집힌다" → "rate=0 자체 기준선 대비 뒤집힌다" |
| 8.2.4 | **서술 강도 완화** | Minor | FFT 듀티비 가설을 단정형→가설형으로, §9 (h)와 정합 |
| 8.4 | **한 줄 추가** | Minor | C 정의 변경(1.316→1.289)이 같은 패턴임을 재확인 |
| 문서 전체 | **부록 색인 추가** | Minor | Appendix C~H 목록을 문서 서두에 배치 |
| Appendix D | **정합성 점검 절차 신설** | 재발 방지 | "절 간 정합성 점검" 방법론 노트 |

**v5 대비 결론의 변화 (요약)**

1. **8.4절이 마침내 §9와 같은 대표 숫자를 쓴다.** C10이 §9·초록에만 적용되고 8.4절 본체는 여전히 540회(64.3%)를 대표로 쓰던 상태가, 4차 리뷰 검증 기준([ ] C12)에서 놓친 채로 통과되어 있었다.
2. **rate별 층화가 집계 순위와 다른 그림을 보인다 — 기하학적 방법에 유리한 방향이다.** rate=0.5%(가장 낮은, 현장에서 가장 있음직한 결손률)에서는 **기하 2단계가 68.5%로 단독 1위**다. 집계(rate 0.5~4% 동일 가중)의 "ACF(전역최대)와 공동 1위"는 이 낮은-결손 구간의 우위를 rate=4%의 열세와 뭉쳐 평균 낸 결과였다. **불리한 결과를 숨기지 않은 것과 같은 기준으로, 이 유리한 결과도 보고한다.** 다만 구현 비용 논거는 유지된다(아래 8.4절 참고).
3. **ACF(전역최대)의 "결손 조건 권고"는 결손율을 안다는 전제에서만 성립한다.** rate=0(클린)에서는 6개 중 4위(45.5%)로 추락하므로, 결손율이 알려지지 않은 상태에서 이 방법을 고르는 것은 위험하다.

### v4 → v5 변경 이력 *(4차 개정, 참고용 보존)*

4차 리뷰의 최우선 과제는 v4가 새로 제시한 헤드라인(결손 조건 순위 재역전)의 근거인
Appendix G 로그에서 **11패턴 중 8개가 rate=0보다 rate>0에서 성공률이 높은** 이상 현상을
규명하는 것이었다(C8). 규명 결과 **코드 버그가 아니라 실재하는 현상**임이 확인되었다 —
clean 데이터에서는 참 주기의 배수들이 median H·ACF 값에서 사실상 동일(tie)한데, 산발적
결손이 그 tie를 깨면서 가장 작은 배수(=대개 참 주기 자신)만 남기기 때문이다. 이는 8.2.5절
본편이 이미 규명한 "결손이 검출을 작은 약수 쪽으로 편향시킨다"는 원리와 **정확히 같은
메커니즘의 반대쪽 얼굴**이다 — 경쟁 상대가 참 주기보다 **큰** 배수이면 결손이 도움이 되고,
경쟁 상대가 참 주기보다 **작은** 약수이면 결손이 해롭다.

| 절 | 조치 | 대응 | 요지 |
|---|---|---|---|
| 8.2.5 | **소절 신설 + 비교 기준선 교체** | C8, C9, M15, M16 | 배수 tie 해소 메커니즘 규명, rate=0 자체 기준선(330회)으로 비교 재구성, 단독 생존 사례 재해석 |
| 8.4 | **표 아래 스코프 설명 추가** | M17 | 권고표 아래에 용도(목적1/목적2)별 상충 설명 각주 추가 *(v6 정정 — M20: 원래 "열 추가"로 오기했으나 실제로는 열이 아니라 표 아래 각주였다)* |
| 9, 초록 | **첫 불릿/헤드라인 교체** | C10 | 물리 기반 세트(990회)를 대표 결과로, 540·270회는 방법론 이력으로 재배치 |
| 9 | **모순 문장 정정** | C11 | FFT "제외가 옳았다"→"조건부 복귀"로, "+5.9%p 유의하지 않음"→8.4절 갱신 결과로 교체 |
| 본문 전체 | **로그 인용 접두어** | M13 | `[로그 Lxx]` → `[로그 C:Lxx]`/`F:Lxx`/`G:Lxx`. 이 과정에서 실제로 잘못된 범위를 가리키던 인용 5곳도 함께 발견해 정정 |
| 8.2.2, 8.3.1, 8.4, 9 | **정의 표기 병기** | M14 | 옛 대비 지표 C 정의를 쓰는 자리에 "(v3 정의)" 표기 |
| Appendix D | **재발 방지 절차 추가** | C8 방법론 노트 | "예상과 반대 방향 결과 자동 점검" 절차 명문화 |
| Change Log | **차수 오기 정정** | Minor | "두 차례"→"네 차례" |

**v4 대비 결론의 변화 (요약)**

1. **v4의 헤드라인(결손 조건 순위 재역전)은 유지되지만, 근거가 재구성되었다.** 올바른 기준선(rate=0, 330회)으로 다시 계산해도 직접위상(일반화) 77.9%→39.7%(−38.2%p)의 붕괴와 기하 2단계 62.7%→60.3%(−2.4%p)의 상대적 안정성은 그대로다. 다만 이는 8.2.4절(990회, 3조건 혼합)과의 비교가 아니라 **8.2.5절 자신의 rate=0 부분집합과의 비교**로 다시 서술되었다.
2. **결손이 성능을 "개선"하는 8개 패턴은 v4가 발견하지 못한 진짜 현상이었다.** M5(드리프트 역설)와 같은 유형의 재발이며, 재발 방지를 위한 점검 절차를 방법론 노트로 남긴다.
3. **§9와 초록의 헤드라인이 마침내 물리 기반 세트(권고의 주 근거)로 정렬되었다** — v4는 8.2.3절에서 위계를 선언해 놓고도 §9 본문은 여전히 무효화된 540회 세트를 대표 결과로 인용하는 모순을 남겼었다.

### v3 → v4 변경 이력 *(3차 개정, 참고용 보존)*
...
**(다) 산발적 결손(tooth loss) 조건의 추가** — 실무 확인 결과 오검출(삽입)은 이 설비에서 실재하지 않지만, 검출 누락(결손)은 현실적 위협이다. 리뷰 항목은 아니지만 클린 데이터 결론(직접위상(일반화) 1위)의 전제를 흔드는 결과가 나와, 8.2절 이후 전체 논조를 이 조건까지 포함하도록 수정한다.

| 절 | 조치 | 요지 |
|---|---|---|
| 8.2.5 | **신규 추가** | 물리 기반 11패턴 × rate(0~4%) × 30회 = 1,650회 결손 실험 + McNemar 검정 |
| 8.4 | **표·항목 교체** | 데이터 신뢰도별 권고표로 재구성, "표준 기법"이 조건에 따라 달라짐을 명시 |
| 9, 초록 | **항목 추가** | 결손 조건에서의 순위 재역전(McNemar로 확정)을 핵심 결론에 반영 |
| Appendix E, G | **신규 추가/확장** | `verification_tooth.py` 등재, 전체 로그를 Appendix G로 첨부 |
...
4. **산발적 검출 누락 조건에서 클린 데이터 순위가 McNemar 검정으로 확정된 채 뒤집힌다.** 기하 2단계는 ACF(전역최대)와 통계적 동률(공동 1위)이 되고, 클린 데이터 1위 직접위상(일반화)을 포함한 나머지 넷을 유의하게 이긴다. 다만 동률 상대가 구현 비용은 훨씬 싸므로, 이 결과가 기하학적 방법의 채택 근거로 이어지지는 않는다 — "표준 기법에 자리를 내주는 것이 합리적"이라는 결론 자체는 유지되며, 다만 어떤 표준 기법인지가 데이터 품질에 따라 달라진다.
...
**[개정 v5 — 대표 결과 재배치 (C10)]** 실무에서 확인한 설비 구조(짝수 주기=2매 동시 생산, 홀수 주기=순차 stage, §1.1)에 직접 근거해 구성한 11패턴×3조건×30회 = **990회**를 이 최종 방법의 대표 검증 결과로 삼는다. 6개 방법이 동일한 데이터셋에서 평가된 대응표본이므로 **McNemar 정확검정 + Holm 보정**으로 비교하였고, 성공은 **반환값이 참 주기와 정확히 일치할 때(strict)**로 정의하되 배수를 인정하는 기준의 결과를 병기하였다. 성공률(strict)은 **직접 위상비교(일반화) 92.1% > ACF(첫국소최대) 81.7% > 기하학적 2단계 68.4% > ACF(전역최대) 49.6% > 직접 위상비교(2배) 40.7% > FFT 34.9%**였다. 즉 **기하학적 방법은 구현 복잡도가 가장 크면서 6개 중 3위**이며, 두 방법(ACF 첫국소최대, 직접위상 일반화)에 유의하게 밀리고 세 방법(ACF 전역최대, 직접위상 2배, FFT)을 유의하게 이긴다.

같은 물리 기반 패턴에 **산발적 검출 누락(결손, rate<5%)**을 섞어 재검증하였다(8.2.5절, 1,650회). 오검출(삽입)은 이 설비에서 실재하지 않음을 실무로 확인해 결손만 다루었다. 결손은 인덱스-위상 대응을 영구히 밀어내는데, 이 밀림이 후보 주기의 약수와 우연히 일치할 확률(1/N)이 작은 N일수록 높아 **검출을 참 주기의 작은 약수 쪽으로 체계적으로 편향**시킨다. 그 결과 클린 기준선(rate=0, 330회) 대비 직접위상(일반화)은 77.9%→39.7%(−38.2%p)로 붕괴하고, **McNemar 대응표본 검정으로 확인하면(n=1,320) 기하학적 방법은 ACF(전역최대)와 통계적 동률(공동 1위, 62.7%→60.3%로 거의 안정)이 되며 나머지 네 방법을 전부 유의하게 이긴다.** 다만 **11패턴 중 8개에서는 결손이 오히려 성공률을 높였다** — clean 데이터에서 참 주기의 배수들이 통계적으로 구분되지 않는(tie) 경우, 결손이 그 tie를 깨면서 가장 작은 배수만 남기기 때문이며, 이는 코드 결함이 아니라 "결손이 작은 약수 쪽으로 편향시킨다"는 같은 원리의 반대쪽 얼굴임을 직접 확인하였다. 저대비 조건과 결손이 겹치면 6개 방법 전부가 예외 없이 0%로 무너지는 시나리오도 존재한다. **[개정 v6]** 이 "공동 1위"는 rate=0.5/1/2/4%를 동일 가중으로 묶은 집계일 뿐이다 — rate별로 쪼개면 **rate=0.5%(현장에서 가장 있음직한 낮은 결손률)에서는 기하학적 방법이 68.5%로 단독 1위**이고, rate=2~4%에서만 ACF(전역최대)가 유의하게 앞선다(8.2.5절, C15). 불리한 결과를 숨기지 않은 것과 같은 기준으로 이 유리한 결과도 보고하되, 실제 현장 결손률을 사전에 알 수 없다는 점에서 "구현이 압도적으로 싼 쪽을 기본값으로 한다"는 결론 자체는 유지된다.

**방법론적 시행착오의 기록**으로, 참 주기 P=4/6/5·고대비/저대비·정상/드리프트/중간시프트의 18개 시나리오×30회(원래 세트, 540회)와 위상 대비를 통제한 재실험(통제 세트, 270회)도 함께 보고한다. 원래 세트의 P=5 패턴은 실제로는 어느 설비 구조에도 대응하지 않음이 이후 밝혀졌고(540회 중 180회, 1/3이 무효), 통제 세트는 6.2절의 홀짝 혼동원까지 제거해 실제 설비 어디에도 대응하지 않는 진단용 세트다. 두 세트에서는 방법 순위가 서로 다르게 재편되며(예: 통제 세트에서 기하학적 방법이 ACF(전역최대)를 유의하게 이기고 FFT가 0%→100%로 뒤집힌다), 이 재편 과정에서 채점 기준(배수 인정 여부)에 따라 순위가 통째로 바뀐다는 것과 FFT의 실패가 옥타브 오류라는 패턴-특이적 현상임이 함께 드러났다. 상세 결과와 대표 세트를 사용하지 않는 이유는 8.2.2~8.2.4절에 있다.

기하학적 접근은 스케일 종속성·과잉 정규화·강건성-검출력 트레이드오프·산발적 결손에 대한 데이터 구조적 취약성이라는 함정을 발견하고 해결하는 **탐구 과정으로서 가치가 있었으나, 실전 배포용 알고리즘으로는 (데이터 품질 조건에 따라 직접위상(일반화) 또는 ACF(전역최대)로 달라지는) 표준 기법에 자리를 내주는 것이 합리적**이라는 것이 본 논문의 결론이다 — 결손 조건에서 통계적으로 동률인 ACF(전역최대)가 구현 비용은 압도적으로 낮기 때문이다. 다만 **이 결론은 전적으로 합성 데이터에 근거한다** — 실측 생산 로그는 사내 비공개 자료여서 수록·검증할 수 없었으며, 이는 본 연구의 가장 큰 외적 타당성 한계다(7.3절, 검증 절차는 7.3.1절에 명세).

---

### 부록 색인 *(v6 신규 — Minor)*

문서 내 부록은 신설된 순서(A→B→C→E→F→G→H, D는 여러 라운드의 체크리스트를 통합하며 문서 맨 뒤로 밀렸다)대로 배치되어 있어 알파벳 순서와 다르다. 아래는 알파벳 순 색인이다.

| 부록 | 내용 | 신설 시점 |
|---|---|---|
| A | 기호 정리 | v1 |
| B | 검증 코드 생성을 위한 Claude Code 프롬프트 | v1, v2에서 확장 |
| C | 실행 로그(원래 세트·통제 세트) | v2 신규, v3에서 전면 교체 |
| D | 리뷰 대응 체크리스트(1~5차 통합) | v2 신규, 매 라운드 갱신, v5에서 4라운드 통합 |
| E | 검증 스크립트 목록 + 커밋 해시 | v3 신규 |
| F | v4 실행 로그(물리 기반 세트) | v4 신규 |
| G | 산발적 결손 실험 로그 | v4 신규 |
| H | C8/C9/M15 재검증 로그 | v5 신규 |
| I | rate별 층화 분석 로그(C15) | v6 신규 |
...
**[개정 v2 추가] 작은 N에서의 근사 타당성**: "대수의 법칙에 의해 Σ⁻ₙ(n) ≈ N·d"라는 서술은 N이 클 때의 점근적 진술이므로, N=4처럼 작은 값에서도 성립하는지 실증이 필요하다. 동일 합성 데이터(N=4 패턴, L=100,000, seed=42)에서 Σ⁻ₙ(n)의 분포를 직접 측정하였다[로그 C:L153-161].
...
S/N, |BC|/N이 N에 관계없이 상수로 유지되는 것이 확인되어(각각 오차 1% 이내, 로그 C:L23-24), 앞선 이론적 분석(S, |BC| 모두 N에 선형 비례)이 실증적으로 뒷받침되었다. **S를 원시(raw) 값 그대로 N 후보 간 비교 지표로 사용할 수 없으며, 정규화가 필요하다.**

> **[개정 v2 각주 — 재현성]** 위 표는 시드를 고정하지 않은 10회 시행의 결과이므로, 재실행하면 소수점 넷째 자리 수준에서 값이 달라진다. **이는 "매 실행마다 다른 난수로도 같은 결론이 나오는지"를 보이기 위한 의도된 설계**다. 시드를 고정한 동일 조건의 재현 가능한 실행은 4.4절/7.1절의 회귀 검정에서 시드 리스트 `[1000..1009]`와 함께 제시한다[로그 C:L8].
...
**[개정 v2 추가] N-불변성의 통계적 검정**: v1은 "median H가 N=4~16에서 0.036 부근으로 일정하다"는 눈대중 비교에 의존했다. 이를 검정으로 대체한다. 시드 `[1000..1009]` 10회 시행에서 N=4,8,12,16의 median H를 구하고 `H ≈ a + b·N` 선형회귀를 적합한 뒤, 시행 단위 부트스트랩(2000회)으로 기울기 b의 95% 신뢰구간을 산출하였다[로그 C:L11-27].
...
> **[개정 v3 — 수치 정정 (M4)]** v2는 이어서 "기울기가 CI 끝값이라 해도 N=4→16 변화는 평균 H의 **0.06%** 미만"이라고 적었으나, 이는 **계산 오류**다. 실제로는 (CI 상한) × ΔN / 평균 H = 2.127×10⁻⁵ × 12 / 0.036138 = **0.71%**로, v2가 약 12배 과소 보고했다. 재계산된 값은 다음과 같다[로그 C:L141-146].
>
...
**[이미지 01]** 위상 대비에 따른 median 판별비 비교 [로그 C:L70-90]
...
**[이미지 02]** 샘플 수(L)에 따른 신뢰구간 수렴 [로그 C:L92-99]
...
**[이미지 03]** 실제 배치 규모에서 mean vs median 신뢰구간 비교 [로그 C:L101-112]
...
**[이미지 04]** H, κ 분포 Box plot (N별) [로그 C:L32-45]
...
[로그 C:L16-21] — **[개정 v3]** 이 표도 4.3절과 같은 무시드 실험에서 나오므로 재실행 값으로 교체하였다(M8).

median H는 N에 무관하게 상수로 유지되었으나, **median κ는 N이 커질수록 급격히 감소**하였다(N=4 대비 (N/4)²로 나누면 대략 일치 — κ가 1/N²에 비례). 각 항의 스케일을 직접 분해하면 원인이 드러난다[로그 C:L52-61]:
...
- **비정상성에 대한 H의 거동은 대신 8절의 합성 실험(사인파 드리프트, 중간 레벨 시프트 × 540회 반복)이 담당한다.** **[개정 v3 — 조건부로 정정 (M5)]** v2는 여기서 "저대비+사인드리프트 20.0%(P=4)"만 인용해 "비정상성 취약성이 재확인되었다"고 적었으나, 이는 유리한 절반만 인용한 것이다. 정확한 서술은 다음과 같다 — **드리프트에 대한 취약성은 저대비 조건에서만 나타나며, 대비가 큰 조건(C=1.316, v3 정의)에서는 오히려 성공률이 80.0% → 100.0%로 개선된다**[로그 C:L395-399]. 진폭을 스윕하면 거동이 비단조임도 확인된다(8.3.1절). 따라서 "H가 비정상성에 취약하다"는 v1의 우려는 **조건부로만 지지되며, 무조건적 일반화는 근거가 없다.**
...
**[개정 v2 — 직접 위상비교의 후보 집합 편향(C2)]** v1에서 사용한 `find_period_direct_phase`는 후보를 N → 2N으로만 배가시킨다. 따라서 N_max=20에서 이 알고리즘이 **반환할 수 있는 값은 {1, 2, 4, 8, 16}, 즉 2의 거듭제곱뿐이며, P=5나 P=6 같은 주기는 원리적으로 반환이 불가능하다**[로그 C:L166-173]. 실제로 P=6 데이터에서는 4를, P=5 데이터에서는 1을 반환했다. v1의 8절 결론이 이 구현의 "6/6 성공"에 상당 부분 의존했으므로, 이는 결론의 유효 범위를 좁혀야 하는 심각한 결함이다.

이를 보정하기 위해 각 단계에서 배수 k = 2, 3, …, ⌊N_max/N⌋을 모두 시도하는 **일반화 버전**을 추가로 구현하였다. 한 단계에서 N·(k−1)회의 다중 비교가 일어나므로 Bonferroni 보정(α_adj = α/(N(k−1)))을 적용했다 — 보정이 없으면 위양성이 누적되어 참 주기의 배수(P=6에서 12, P=5에서 15)로 과도 세분화되는 현상을 확인했다. 일반화 버전은 P=6에서 6을, P=5에서 5를 정확히 반환한다[로그 C:L171-172]. 8.2절에서는 두 버전을 모두 비교 대상에 포함한다.
...
**[개정 v3 — 패턴 난이도의 정량화 (M6)]** "고대비/저대비"는 v2에서 정성적 이름표에 불과했다. 위상 대비를 **C = min|μᵢ − μⱼ| / mean(μ)** (큰 위상 평균들 사이의 최소 상대 간격, C가 작을수록 어려움)로 정의해 6개 패턴의 난이도를 측정하면 다음과 같다[로그 C:L409-417].

> **[개정 v5 — 정의 표기 (M14)]** 아래 C 값은 **미세 딜레이(0.1)를 제외**하고 "큰 위상" 값들 사이에서만 계산한 v3 정의다. 8.2.4절은 카세트 지연형 패턴에서 "지연시간이 미세 딜레이에 얼마나 가까운가"도 난이도이므로 미세 딜레이를 포함하는 정의로 바꾸었다(그 결과 예컨대 P=4 high는 1.316→1.289로 미세하게 바뀐다). 이 절과 8.3.1절·8.4절 4항·§9가 인용하는 C 값은 모두 **이 v3 정의**이며, 8.2.4절·8.2.5절이 인용하는 C 값은 **v4 정의**다. 대소 관계와 결론은 어느 정의로도 바뀌지 않으므로 재계산해 통일하지 않고 표기로 구분한다.

| 패턴 | 큰 위상 평균 | 최소 간격 | mean(μ) | **C (v3 정의)** |
|---|---|---|---|---|
...
> 모든 저장 경로가 로그에 상대경로로 기록되어 있다[로그 C:L45, C:L90, C:L99, C:L112, F:L204, C:L401, C:L447].
...
**시나리오별 성공률 (%) [Wilson 95% CI], strict 기준, n=30** [로그 C:L206-227]
...
**전체 540회 시행 기준 총계 — strict / lenient 병기** [로그 C:L235-259]
...
**참 주기별 소계 (%)** [로그 C:L243-246, C:L256-259]
...
주기 효과와 난이도 효과를 분리하기 위해, **세 주기에서 대비 지표를 동일하게 맞춘 패턴 세트**를 설계해 실험을 다시 돌렸다. 각 주기의 P개 위상 평균을 mean(μ)=6.0을 중심으로 간격 1.2의 등차수열로 배치하면 P에 관계없이 C=0.200이 된다[로그 C:L422-425]. 표준편차는 기존과 같이 평균의 13.3%로 둔다.
...
3주기 × 3조건 × 30회 = **270회 시행** 결과(v4에서 동일 조건으로 재실행, v3과 수치 일치)[로그 F:L96-163]:
...
**[개정 v4 — strict / lenient 병기 및 반환값 분포 (M9)]** v3은 이 세트에 대해 strict 총계와 P별 소계만 실었다. 결론을 뒤집을 수 있는 실험일수록 보고가 얇아서는 안 된다는 지적을 반영해, 원래 세트와 같은 수준으로 보강한다[로그 F:L109-163].
...
**반환값 분포** (전 조건 합산, n=90/주기)[로그 F:L142-163] — 시나리오별 표는 로그 F:L112-122에 있다.
...
**1. 기하학적 방법의 ACF(첫국소최대) 대비 열세는 대비를 맞춰도 사라지지 않는다 — 오히려 커진다.** 격차가 원래 세트의 +11.9%p에서 **+26.3%p**로 벌어졌고 McNemar Holm p = 3.54×10⁻¹⁴다[로그 F:L170]. 이 방향에서는 v2·v3의 결론이 유지된다.
...
**4. 기하 2단계의 P=6 0.0%는 지표가 아니라 판정 규칙의 실패다 (M11).** v3은 이 극단적 실패를 진단하지 않고 향후 과제로 미뤘다. 8.3절의 진단 도구를 그대로 적용하면 원인이 곧바로 드러난다[로그 F:L183-208].
...
§1.1의 설비 구조에서 패턴을 직접 구성해 다시 검증한다. **짝수 주기는 2매 세트 구조, 홀수 주기는 순차 stage 구조**에서만 나오므로, 주기마다 대응하는 형태가 정해진다[로그 F:L213-239].
...
**설비 유형별 성공률 (strict)** [로그 F:L283-291]
...
**McNemar + Holm (990회 대응표본, strict)** [로그 F:L320-324]
...
> **[개정 v5 — FFT의 P=3 vs P=5 성능 차이 진단 (Minor)] [개정 v6 — 서술 강도 완화, §9 (h)와 정합]** 위 표에서 FFT는 같은 순차 stage 구조인데 P=3 100%와 P=5 42.2%로 크게 갈린다. 실패 사례를 직접 확인하면 단서가 드러난다 — P=5가 실패할 때 FFT는 예외 없이 **정확히 2**를 반환한다(고대비/저대비, 정상/드리프트/중간시프트 전 조건에서 동일). 이는 펄스형 신호의 듀티비(duty cycle) 문제로 **설명될 수 있다는 가설이다**: 순차 stage 패턴은 P개 위상 중 1개만 크고 나머지 (P−1)개는 균일한 "좁은 펄스" 신호이며(듀티비 1/P), 구형파 펄스열의 n차 조화파 세기가 sinc(n/P)에 비례한다는 이론에 따르면 **듀티비가 작을수록(P가 클수록) 에너지가 기본 주파수(period=P)보다 높은 조화파 쪽으로 밀릴 수 있다.** P=3(듀티비 1/3)은 기본 주파수가 여전히 우세하지만, P=5(듀티비 1/5, 더 좁은 펄스)는 2차 조화파(주파수 2/P, 정수 주기로 반올림하면 2)가 기본 주파수를 앞지르는 경우가 관측상 절반 이상이다. 즉 **P=5의 FFT 실패는 다른 실패들과 같은 옥타브(고조파) 오류의 한 변종일 가능성이 있으나**, 이는 "정확히 2를 반환한다"는 관측과 sinc 이론에 기댄 정성적 설명일 뿐이며 **정량 검증(펄스 폭을 체계적으로 바꾼 재실험)은 아직 수행하지 않은 남은 과제다**(§9 (h)).

##### 오경보 / 미탐 분석 — 알림 응용의 결정적 지표

§1.2 목적 2를 직접 측정한다. 참 P=2(정상 설비)에서 짝수 배수를 반환하면 **오경보**, 참 P=4(교체 지연 발생)에서 1이나 2를 반환하면 **미탐**이다[로그 F:L339-353].
...
#### 8.2.5 산발적 결손(tooth loss)에 대한 강건성 *(v4 신규)*

지금까지의 비정상성 테스트(사인 드리프트, 중간 레벨 시프트)는 값이 바뀔 뿐 인덱스 n과 물리적 위상(n mod P)의 대응관계는 유지되는 경우였다. 그러나 실제 검출 센서는 드물게 이벤트를 통째로 놓칠 수 있다 — 로그 한 줄이 아예 찍히지 않는 **결손**이다. 결손이 한 번 일어나면 그 지점 이후 전체 구간에서 인덱스-위상 대응이 **영구히 한 칸 밀린다.** 실무 확인 결과 이 설비에서 오검출(로그에 없던 이벤트가 잘못 끼어드는 삽입)은 실재하지 않으므로, 결손만 다룬다.

**설계**: 8.2.4절의 물리 기반 11개 패턴 전체(정상 조건)에 대해, 각 tact 위치를 독립적으로 확률 rate로 판정해 결손시킨다(고정 개수가 아니라 확률로 판정하므로 실제 발생 개수는 이항분포를 따르는 확률변수다). rate = 0%, 0.5%, 1%, 2%, 4%(모두 5% 미만), 각 조합 30회 반복 — 11패턴 × 5rate × 30회 = **1,650회 시행**[로그 G:L52-106].

**메커니즘**: 결손 하나가 만드는 인덱스 밀림(shift)은 후보 주기 N에 대해 shift mod N = 0일 때만 "안 보이는" 오차가 된다. 이 확률은 1/N이므로 **작은 N일수록 원천적으로 결손에 안전**하다 — 이는 특정 알고리즘의 결함이 아니라 데이터의 잔차 구조 문제이므로 ACF·직접 위상비교·기하학적 방법을 가리지 않고 똑같이 작용한다. 실제로 median H를 비교하면 이 비대칭이 드러난다(seed=6000, rate=0.5%, 결손 93개)[로그 G:L13-31]:

| 패턴 | N | clean median H | 결손 후 median H | 변화율 |
|---|---|---|---|---|
| 저대비(10,0.1,9,0.1) | 2(약수) | 0.0392 | 0.0591 | 1.51x |
| 저대비(10,0.1,9,0.1) | 4(참주기) | 0.0383 | 0.1017 | **2.65x** |
| 고대비(10,0.1,5,0.1) | 2(약수) | 0.0844 | 1.9390 | 22.97x |
| 고대비(10,0.1,5,0.1) | 4(참주기) | 0.0364 | 0.0675 | **1.85x** |

저대비 패턴은 clean 상태에서 이미 N=2와 N=4의 median H 격차가 0.0392 대 0.0383으로 거의 없다(6.2절에서 이미 지적한 저대비 혼동). 결손이 N=4를 N=2보다 훨씬 심하게 악화시키므로(2.65배 대 1.51배), 이 razor-thin 격차가 즉시 뒤집혀 N=2로 오판된다. 고대비 패턴은 clean 격차가 넓어서(0.0844 대 0.0364) N=2가 훨씬 더 심하게 망가져도(23배) N=4가 여전히 이긴다 — 그래서 고대비는 rate 1~2%까지 버틴다.

**[이미지 10]** 결손 비율(rate)에 따른 11개 패턴의 검출 성공률

![이미지 10](image/이미지_10_산발적결손_강건성.png)

**결과 — rate=0 자체 기준선 대비 순위가 다시 한번 뒤집힌다 (C9)** [로그 G:L169-174]

| 방법 | 전체(rate>0, n=1,320) | 짝수 주기·2매세트/카세트지연(n=840) | 홀수 주기·순차stage(n=480) |
|---|---|---|---|
| ACF(전역최대) | **62.0%** [59.3, 64.6] | **50.0%** | 82.9% |
| 기하 2단계 | **60.3%** [57.6, 62.9] | 41.3% | 93.5% |
| ACF(첫국소최대) | 50.3% [47.6, 53.0] | 22.1% | **99.6%** |
| FFT | 43.4% [40.8, 46.1] | 14.3% | 94.4% |
| 직접위상(일반화) | 39.7% [37.1, 42.4] | 26.9% | 62.1% |
| 직접위상(2배) | 14.0% [12.2, 16.0] | 22.0% | 0.0% |

**[개정 v5 — 비교 기준선의 정정 (C9)]** v4는 여기서 8.2.4절의 990회(정상+드리프트+중간시프트 3조건 혼합)를 "클린 순위"로 인용해 비교했다. 그러나 8.2.5절 자신은 **정상 조건만** rate로 변주하므로, 조건 구성이 다른 990회와의 비교는 apples-to-apples가 아니었다(M16). 올바른 기준선은 **8.2.5절 자신의 rate=0 부분집합(11패턴×30회 = 330회, 정상 조건)**이다[로그 H:L68-91]:

| 방법 | rate=0 기준선(n=330) | rate>0(n=1,320) | 변화 |
|---|---|---|---|
| 직접위상(일반화) | 77.9% [73.1, 82.0] | 39.7% [37.1, 42.4] | **−38.2%p (붕괴)** |
| ACF(첫국소최대) | 75.2% [70.2, 79.5] | 50.3% [47.6, 53.0] | −24.9%p |
| 기하 2단계 | 62.7% [57.4, 67.8] | 60.3% [57.6, 62.9] | **−2.4%p (거의 안정)** |
| ACF(전역최대) | 45.5% [40.2, 50.8] | 62.0% [59.3, 64.6] | +16.5%p |
| FFT | 39.7% [34.6, 45.1] | 43.4% [40.8, 46.1] | +3.7%p |
| 직접위상(2배) | 38.5% [33.4, 43.8] | 14.0% [12.2, 16.0] | −24.5%p |

**올바른 기준선으로 다시 봐도 핵심 결론은 그대로다** — 클린 데이터 1위였던 직접위상(일반화)이 결손 앞에서 가장 크게 붕괴하고(−38.2%p), 기하 2단계는 클린 데이터에서 이미 6개 중 3위(68.4%, 8.2.4절 990회 기준)에 그쳤던 것과 달리 **자기 자신의 클린 기준선(62.7%) 대비 거의 흔들리지 않는다(−2.4%p).** 다만 v4가 인용한 "92.1%→39.7%"라는 낙차는 서로 다른 실험 조건(3조건 혼합 990회 vs 정상 전용 1,320회)을 비교한 것이었으므로, 정확한 낙차는 **77.9%→39.7%**다. 원인은 8.2.1절에서 이미 지적한 구조 그대로다 — 직접위상(일반화)은 **전역 인덱스 스트라이드**(`y[i::M]`)에 의존하므로, 결손 한 번으로 인덱스-위상 대응이 전역적으로 밀리면 스트라이드 그룹 자체가 통째로 오염된다.

**[McNemar 검정 — Wilson CI 겹침이 아니라 대응표본으로 확인]** 6개 방법은 rate>0의 모든 시행에서 **동일한 데이터셋**을 평가했으므로, C3·C5와 같은 이유로 marginal Wilson CI의 겹침이 아니라 대응표본 검정으로 순위를 확정해야 한다. 기하 2단계를 기준으로 나머지 5개 방법과 McNemar 정확검정 + Holm 보정을 수행하였다(n=1,320)[로그 G:L178-189]:

| 비교 (기하2단계 vs B) | 차이 (B−기하2단계) | Holm p | 판정 |
|---|---|---|---|
| **ACF(전역최대)** | **+1.7%p** | 1.47×10⁻¹ | **유의하지 않음 — 통계적으로 동률** |
| ACF(첫국소최대) | −10.0%p | 8.48×10⁻²³ | 기하2단계가 유의하게 우세 |
| FFT | −16.9%p | 8.09×10⁻⁴⁶ | 기하2단계가 유의하게 우세 |
| 직접위상(일반화) | −20.6%p | 2.28×10⁻⁴⁴ | 기하2단계가 유의하게 우세 |
| 직접위상(2배) | −46.3%p | 1.03×10⁻¹⁷⁰ | 기하2단계가 유의하게 우세 |

**즉 산발적 결손 조건에서 기하 2단계는 ACF(전역최대)와 통계적으로 구별되지 않는 공동 1위이며, 나머지 4개 방법(클린 데이터 1위였던 직접위상(일반화) 포함)을 전부 유의하게 이긴다.** 이는 우연한 raw 퍼센트 차이가 아니라 대응표본 검정으로 확정된 결과다.

##### rate별 층화 — 집계가 감춘 두 갈래 힘 (v6 신규, C15)

**C8이 확정한 메커니즘은 방향이 반대인 두 가지다** — 결손은 (i) 작은 약수 쪽으로 오탐을 편향시키기도 하고(성능을 해침), (ii) clean 상태에서 배수끼리 이루던 tie를 깨 참 주기를 살리기도 한다(성능을 도움, C8). 위 집계(n=1,320)는 rate=0.5/1/2/4%를 **동일 가중으로 묶은 값**이므로, 두 메커니즘의 세기가 rate마다 다르게 작용한다면 그 차이를 평균으로 지워버릴 수 있다. 같은 실험을 rate별로 나눠 재실행해 확인하였다(11패턴×30회=330회/rate, Appendix G·H와 동일 시드 — 집계치가 일치함을 직접 확인)[로그 I:L11-65].

| rate | 기하2단계 | ACF(첫국소최대) | ACF(전역최대) | FFT | 직접위상(2배) | 직접위상(일반화) | 1위 |
|---|---|---|---|---|---|---|---|
| 0.0% | 62.7 | 75.2 | 45.5 | 39.7 | 38.5 | 77.9 | 직접위상(일반화) |
| 0.5% | **68.5** | 54.8 | 61.2 | 43.9 | 21.2 | 58.5 | **기하2단계 (단독)** |
| 1.0% | 61.8 | 54.5 | 64.2 | 42.4 | 17.0 | 43.9 | ACF(전역최대) |
| 2.0% | 62.1 | 46.4 | 68.8 | 44.5 | 9.1 | 33.0 | ACF(전역최대) |
| 4.0% | 48.8 | 45.5 | 53.6 | 42.7 | 8.8 | 23.3 | ACF(전역최대) |

**[이미지 11]** rate별 층화 성공률 (동일 가중 집계와 달리 rate마다 순위가 다르다)

![이미지 11](image/이미지_11_rate별_층화_성공률.png)

[로그 I:L70-76]. 전체 순위도 rate마다 달라진다: 0.0%는 직접위상(일반화)>ACF첫국소>기하2단계>ACF전역최대>FFT>직접위상(2배); 0.5%는 **기하2단계**>ACF전역최대>직접위상(일반화)>ACF첫국소>FFT>직접위상(2배); 1.0~4.0%는 모두 ACF전역최대>기하2단계>ACF첫국소>…순이다[로그 I:L78-83].

**세 가지가 드러난다.**

1. **rate=0.5%(현장에서 가장 있음직한 낮은 결손률)에서는 기하 2단계가 68.5%로 단독 1위다.** 집계의 "ACF(전역최대)와 공동 1위" 프레이밍은 이 낮은-결손 구간의 우위를 rate=4%의 열세와 뭉쳐 평균 낸 결과였다. **불리한 결과를 숨기지 않은 것과 같은 기준으로, 이 유리한 결과도 보고한다.**
2. **ACF(전역최대)는 rate 0→2%에서 단조 개선한다(45.5→61.2→64.2→68.8), rate=4%에서는 꺾인다(53.6)**[로그 I:L86-88]. 이는 C8의 "배수 tie 해소"가 결손률이 커질수록 더 확실히 작동하다가, 결손이 너무 잦아지면(4%) 참 주기 자신의 신호까지 함께 훼손되기 시작하는 것으로 해석할 수 있다(정량 검증은 남은 과제, §9 (h)와 같은 유형의 가설).
3. **기하 2단계 vs ACF(전역최대)의 rate별 McNemar 검정(5개 rate, Holm 보정)은 "공동 1위"가 평평한 동률이 아니라 rate에 따라 방향이 바뀌는 경계선임을 보여준다**[로그 I:L92-103]:

| rate | 기하만 성공 | ACF전역만 성공 | 차이(ACF전역−기하) | 95% CI | Holm p | 판정 |
|---|---|---|---|---|---|---|
| 0.0% | 118 | 61 | −17.3%p | [−25.2, −9.7] | 1.23×10⁻⁴ | **기하2단계 유의 우세** |
| 0.5% | 58 | 34 | −7.3%p | [−13.0, −1.5] | 3.20×10⁻² | **기하2단계 유의 우세** |
| 1.0% | 24 | 32 | +2.4%p | [−2.1, +7.0] | 3.50×10⁻¹ | 동률 (n.s.) |
| 2.0% | 11 | 33 | +6.7%p | [+2.7, +10.6] | 3.78×10⁻³ | **ACF(전역최대) 유의 우세** |
| 4.0% | 1 | 17 | +4.8%p | [+2.4, +7.3] | 5.80×10⁻⁴ | **ACF(전역최대) 유의 우세** |

**즉 rate=0(클린)~0.5%에서는 기하 2단계가 유의하게 우세하고, rate=1.0%에서 승패가 통계적으로 갈리지 않으며(양쪽 다 유의하지 않음), rate=2~4%에서는 ACF(전역최대)가 유의하게 우세로 돌아선다.** 집계(n=1,320, rate=0.5/1/2/4% 동일 가중, Holm p=0.147)의 "동률" 판정은 이 승–무–패 구간을 평균 낸 결과였지, 모든 rate에서 실제로 동률이었다는 뜻이 아니다.

**이것이 8.4절의 "구현 비용이 결정적"이라는 결론을 뒤집는가? — 부분적으로만 그렇다.** rate=0.5% 구간만 본다면 기하 2단계가 통계적으로 우세하므로 구현 비용 논거 자체가 적용되지 않는다(성능이 이미 이기고 있다). 그러나 **실제 결손률을 사전에 알 수 없다는 것이 이 절 전체의 전제**다 — 결손률이 0.5%인지 2%인지 모르는 상태에서 방법을 골라야 한다면, 5개 rate에 걸쳐 승패의 부호 자체가 바뀌므로 "어느 쪽이 이긴다"고 단정할 근거가 없다. 이 부호 불확실성이 오히려 구현 비용이 낮은 쪽(ACF전역최대)을 미는 논거가 된다 — 다만 이는 원래 논거("성능이 같으니 싼 쪽")와는 **다른 논거("성능의 우열 자체가 불확실하니 싼 쪽")**이며, 8.4절 권고표에 이를 구분해 반영한다.

> **[한계 — M19]** 위 집계와 개별-rate 요약 모두 rate=0.5/1/2/4%에 **동일 가중**을 부여한다. 이 가중은 실측 근거가 아니라 실험 설계상 편의적 선택이며, 현장의 실제 결손률 분포는 아직 확보하지 못했다(7.3절/7.3.1절 M1·M7 — 이번 개정 범위 밖, 그대로 유지). **만약 실제 현장 결손률 추정치가 확보되면, 위 동일-가중 집계 대신 해당 rate의 행을 직접 참조해야 한다** — 예컨대 현장 결손률이 0.5% 부근으로 확인된다면 위 표는 "기하 2단계가 우세"를 가리키고, 2% 이상으로 확인된다면 "ACF(전역최대)가 우세"를 가리킨다.

**저대비와 결손이 겹치면 방법과 무관하게 완전히 무너진다.** P=4 2매세트-low, P=6 2매세트-low는 rate≥0.5%에서, P=4 카세트지연-low(6.2절의 조기경보 시나리오)는 rate≥1.0%에서 **6개 방법 전부가 예외 없이 0%**로 붕괴한다[로그 G:L73-76, G:L84-86, G:L103-106].

**[개정 v5 — 단독 생존 사례의 재해석 (M15)]** v4는 "P=4 카세트지연-low, rate=0.5%에서 기하 2단계 홀로 100%(나머지는 0%)"를 국소 이웃만 보는 기하학적 지표의 강점 사례로 기록했다. 그러나 같은 패턴의 **rate=0 기준선을 확인하면 기하 2단계는 이미 73.3%였다**[로그 G:L79] — 0%에서 결손 덕에 살아난 것이 아니라, **원래도 상대적으로 좋았던 성능이 결손 앞에서 거의 그대로(73.3%→100%) 유지된 반면, 나머지 다섯 방법은 각자의 양호했던 기준선(ACF첫국소 100%, 직접위상2배·일반화 각 80%대)에서 rate=0.5%라는 낮은 문턱을 넘자마자 전부 0%로 급락한 것**이다. 이는 C8의 "clean tie 해소" 메커니즘과는 다른 현상이다 — C8은 clean 상태의 나쁜 성능(배수 혼동)이 결손으로 개선되는 경우이고, 이 사례는 **애초에 양호했던 기하 2단계가 완만하게 열화되는 동안 나머지가 급격한 절벽(cliff)을 보이는 경우**다. 단일 시나리오·단일 rate의 결과라 일반화하기는 이르며, 이 절벽 현상 자체의 원인 규명은 남은 과제로 남긴다(9절 (g)).

**홀수 주기(순차stage)는 결손에 전반적으로 강하다** — 나눠떨어질 작은 약수가 없으므로(8.4절 "P=5가 쉬운 이유"와 같은 논리) shift가 우연히 안전해지는 경로 자체가 없다. 짝수 주기(2매세트/카세트지연)는 N/2라는 명백한 탈출구가 있어 훨씬 취약하다.

##### 결손이 성능을 "개선"하는 경우 — 8/11 패턴에서 관측 (v5 신규, C8)

Appendix G 로그를 rate=0 대비로 다시 훑으면, **11개 패턴 중 8개에서 적어도 한 방법 이상이 rate=0보다 rate>0에서 더 높은 성공률**을 보인다[로그 H:L156-190] — 예상(결손은 항상 해롭다)과 정반대 방향이다. 3차 리뷰의 M5(드리프트 역설)와 같은 유형의 재발이므로, 먼저 코드 결함 여부부터 확인하였다.

**(1) 코드 경로 점검**: rate>0인데 실제로 글리치가 0개 뽑힌 시행(200회 중 177회)에서, 그 데이터가 rate=0 경로의 원본과 배열 단위로 완전히 동일함을 확인하였다[로그 H:L6-11]. 즉 rate=0과 rate>0은 "몇 개가 삭제되는가"만 다를 뿐 같은 코드 경로를 공유하며, **코드 버그가 아니다.**

**(2) 메커니즘**: 8.2.5절 본편이 규명한 "shift mod N = 0 확률 1/N" 원리에는 반대쪽 얼굴이 있다. **clean 데이터에서는 참 주기 P의 모든 배수(2P, 3P, …)가 노이즈 외에는 완전히 동일한 값을 반복하므로, median H나 ACF가 배수마다 사실상 tie를 이룬다.** "첫 국소최솟값"·"전역최댓값" 규칙은 이 tie를 표본잡음으로 임의 선택하므로 결과가 사실상 무작위가 된다. 산발적 결손은 배수가 클수록 더 크게 악화시키므로, tie가 깨지면서 **가장 작은 배수(대개 참 주기 자신)만 유일한 최솟값/최댓값으로 남는다.** P=2 패턴(seed=6000)으로 직접 확인하면[로그 H:L14-47]:

| N | clean median H | 결손 후(rate=0.5%) | | lag | clean ACF | 결손 후 |
|---|---|---|---|---|---|---|
| 2(참주기) | 0.0383 | 0.0547 | | 2 | +0.9652 | +0.9416 |
| 4(배수) | 0.0385 | 0.1081 | | 4 | +0.9649 | +0.9167 |
| 8(배수) | 0.0387 | 0.2082 | | 8 | +0.9652 | +0.8718 |
| 14(배수) | 0.0384 | 0.3702 | | 14 | +0.9643 | +0.8069 |

clean에서는 N=2,4,8,14의 median H가 사실상 동일(0.0383~0.0387)하지만, 결손 후에는 N이 커질수록 단조 증가한다 — ACF도 동일한 패턴(단조 감소)이다. 실제로 10개 시드에서 기하 2단계·ACF(전역최대) 모두 clean에서는 2,4,8,10,12 등 제각각을 반환하다가 결손 후에는 **예외 없이 2를 반환**한다[로그 H:L54-65].

**(3) 개선 사례 전수 보고**: [로그 H:L160-179]

| 패턴 | 방법 | rate=0 | rate>0 평균 | 변화 |
|---|---|---|---|---|
| P=2 2매세트-단일 | 기하2단계 | 43.3% | 100.0% | +56.7%p |
| P=2 2매세트-단일 | ACF(전역최대) | 40.0% | 100.0% | +60.0%p |
| P=3 순차stage-high | ACF(전역최대) | 16.7% | 100.0% | +83.3%p |
| P=3 순차stage-low | ACF(전역최대) | 3.3% | 59.2% | +55.8%p |
| P=4 2매세트-high | ACF(전역최대) | 53.3% | 100.0% | +46.7%p |
| P=5 순차stage-high | ACF(전역최대) | 43.3% | 100.0% | +56.7%p |
| P=5 순차stage-low | ACF(전역최대) | 33.3% | 72.5% | +39.2%p |
| (그 외 12건, 기하2단계·ACF첫국소최대·FFT·직접위상일반화 포함) | | | | +5.0~+43.3%p |

**개선이 하나라도 관측된 패턴은 11개 중 8개**(P=2 2매세트, P=3 순차stage 고/저, P=4 2매세트-high, P=4 카세트지연-high, P=5 순차stage 고/저, P=6 2매세트-high)이며, ACF(전역최대)가 가장 자주·가장 크게 개선되었다. 이는 ACF(전역최대)가 clean 데이터에서 배수 오탐에 가장 취약한 방법이었다는 점(8.2.3절)과 정확히 들어맞는다 — **잃을 것이 가장 많았던 방법이 tie 해소의 수혜도 가장 크게 받는다.**

> **[한계]** 본 절은 §1.2 목적 1(1매당 시간 산출) 관점의 성공률만 측정하였고, 8.2.4절과 달리 **정상 조건만 사용**해 드리프트·중간시프트를 결손과 함께 다루지 않았다(C9, M16) — 8.2.4절과의 직접 비교가 불가능한 이유이자, 두 세트를 각자의 자체 기준선으로만 비교하는 근거다. 목적 2(설비 이상 알림, 오경보/미탐)를 결손 조건에서 재평가하는 것과 드리프트·중간시프트를 결손과 결합하는 확장은 비용 대비 우선순위가 낮다고 판단해 남은 과제로 둔다(9절).

### 8.3 실패 원인 진단: 지표가 아니라 판정 규칙 *(v2에서 확장)*

각 방법이 실패할 때 **무엇을 반환하는지**를 보면 원인이 드러난다(정상 조건, 15회)[로그 C:L331-353].
...
- **FFT (0%)**: 모든 조건에서 2를 반환한다. "큰 값, 작은 값이 교대"하는 구조에서 2차 조화파(period=2)가 기본파보다 강하기 때문에 생기는 **옥타브 오류(octave error)**로, 오디오 피치 검출에서 흔한 문제다. 배음 구조를 고려한 보정을 하지 않은 단순 최대 피크 구현이 원인이다. **[개정 v6 — C13]** 이 옥타브 오류 진단 자체는 유지되지만, 이 절이 v3 이후 쓰던 "최종 권고에서 제외한다"는 결론은 8.2.3절·8.2.4절에서 통제 세트(0%→100%)와 물리 기반 세트(순차 stage 71.1%)로 반증되었다 — FFT의 실패는 무능이 아니라 설비 유형(2매 동시 생산의 교대 구조) 의존성이었다. **최종 결론은 8.4절 6항의 조건부 복귀를 따른다**: 옥타브 오류가 나는 2매 세트 라인과 알림 용도에서는 배제하되, 순차 stage 라인의 시간 산출 용도에는 권고한다.
- **직접위상(2배) (28.9%)**: P=6에서 2를, P=5에서 1을 반환한다. 8.1절에서 규명한 대로 2의 거듭제곱만 반환 가능한 구조적 한계다.
- **ACF의 P=6 실패는 ACF 지표의 문제가 아니라 판정 규칙의 문제다.** P=6 데이터의 ACF 곡선은 lag 2·4에서 +0.763, **lag 6·12·18에서 +0.967**로 참 주기 신호가 뚜렷하게 분리되어 있다. 그런데 "상위 20% 이내의 첫 국소최댓값" 규칙(5.2절 순차 탐색 철학을 그대로 이식한 것)의 임계값이 +0.7637이어서, 임계값에 아슬아슬하게 걸치는 lag 2·4를 먼저 채택해버린다[로그 C:L355-359]. 실제로 규칙만 "전역 최댓값"으로 바꾼 ACF(전역최대)는 P=6에서 30.0% → 72.8%로 성능이 뒤집힌다.
...
**상승/하락은 대비에 따라 갈린다**[로그 C:L395-399]. 진폭 0 → 3에서 기하 2단계의 변화:
...
**상승은 대비가 가장 큰 조건(C=1.316, v3 정의) 하나에서만 일어난다.**

**진폭 스윕에 의한 진단.** 드리프트 진폭을 0, 1, 3, 5, 10으로 바꿔가며 30회씩 재실행하였다[로그 C:L370-393].
...
**[개정 v6 — 대표 결과의 교체 (C12)]** v5는 §9와 초록의 헤드라인을 물리 기반 세트로 교체했으나(C10), 그 결론을 요약하는 8.4절 본체는 여전히 무효화된 540회 세트로 시작하는 모순을 남겼다. §9 스스로 "총계 64.3%는 부풀려져 있었다"고 적으면서 8.4절이 그 64.3%를 대표로 쓰는 것은 자기모순이다. 이번 개정에서 §9와 동일한 순서 — **대표 결과(물리 기반 세트) → 방법론적 시행착오의 기록(원래·통제 세트)** — 로 맞춘다.

**대표 결과: 설비 물리 기반 검증 (8.2.4절, 990회, 정상+드리프트+중간시프트)**

성공률(strict)은 **직접위상(일반화) 92.1% > ACF(첫국소최대) 81.7% > 기하 2단계 68.4% > ACF(전역최대) 49.6% > 직접위상(2배) 40.7% > FFT 34.9%**다. McNemar+Holm 검정(8.2.4절 표 재인용)[로그 F:L320-324]:

| 비교 (기하2단계 vs B) | 차이 (B−기하) | Holm p | 판정 |
|---|---|---|---|
| ACF(첫국소최대) | +13.3%p | 1.20×10⁻¹⁶ | **기하가 유의하게 열세** |
| 직접위상(일반화) | +23.7%p | 1.30×10⁻³⁹ | **기하가 유의하게 열세** |
| ACF(전역최대) | −18.8%p | 7.85×10⁻¹⁶ | **기하가 유의하게 우세** |
| 직접위상(2배) | −27.7%p | 3.14×10⁻³⁶ | **기하가 유의하게 우세** |
| FFT | −33.4%p | 1.17×10⁻⁵¹ | **기하가 유의하게 우세** |

**즉 기하 2단계는 6개 중 3위이며, 두 방법(ACF 첫국소최대, 직접위상 일반화)에 유의하게 밀리고 세 방법(ACF 전역최대, 직접위상 2배, FFT)을 유의하게 이긴다.** "표준 기법에 밀린다"는 서술은 이 두 방법에 한정해야 정확하다.

**결손 조건 (8.2.5절)**: 같은 물리 기반 패턴에 산발적 결손을 섞으면 rate별로 순위가 다시 갈린다 — 집계(rate 0.5~4% 동일 가중, n=1,320)에서는 기하 2단계가 ACF(전역최대)와 통계적 동률(공동 1위)이지만, **rate=0.5%(현장에서 가장 있음직한 낮은 결손률)에서는 기하 2단계가 68.5%로 단독 1위**다(C15, 상세는 8.2.5절). 클린 데이터 1위 직접위상(일반화)은 자신의 클린 기준선(77.9%) 대비 39.7%로 붕괴한다.

**방법론적 시행착오의 기록 (원래 세트 540회 / 통제 세트 270회)** *(항 번호는 v2~v5부터 유지 — 다른 절·Appendix D가 "8.4절 N항"으로 인용한다)*

아래 7개 항목은 **더 이상 권고의 근거가 아니며**, 검증 방법이 개선되어 온 과정을 정직하게 남기기 위해 유지한다. 6개 방법이 동일 데이터셋에서 평가된 대응표본이므로 **McNemar 정확검정 + Holm 보정**을 썼다(v2가 쓴 Wilson 겹침 판정은 부적절 — C3)[로그 C:L266-294]. **유효 범위**: 원래 세트는 짝수 주기(P=4, P=6)에만 대응하고(P=5는 물리적으로 무효, §1.1), 통제 세트는 진단용이라 실제 설비 어디에도 대응하지 않는다.
...
1. **기하학적 2단계(64.3%)는 ACF(첫국소최대)(76.1%)보다 유의하게 낮았다** — McNemar Holm p = 3.85×10⁻⁸, 차이 +11.9%p [+7.8, +15.7]. 최고 성능인 직접위상(일반화)(91.9%)과는 +27.6%p [+22.8, +32.4], Holm p = 1.16×10⁻²⁵로 격차가 더 컸다. 이 결과 자체는 원래 세트 내에서는 정확했으나, 위 "**유효 범위**"에 적었듯 세트 자체가 물리적으로 무효한 P=5를 1/3 포함하고 있어 대표 결과로 쓸 수 없다는 것이 C12의 요지다.
2. **다만 모든 표준 기법에 밀리는 것은 아니었다 — 세트를 바꾸면 부호가 뒤집혔다(v4, C5).** 원래 세트에서 기하 2단계 vs ACF(전역최대)는 차이 +5.9%p로 Holm 보정 후 **유의하지 않았다**(p = 0.071). 그런데 통제 세트에서는 **기하 2단계가 −26.7%p로 유의하게 이겼다**(Holm p = 1.69×10⁻⁸). v3은 이 반전을 어디에도 적지 않았었다.

   **두 세트의 McNemar 대조 (strict, 차이는 B − 기하2단계)** [로그 F:L166-180]
   ...
   **부호가 뒤집히는 두 쌍의 공통 원인은 "패턴 구조에 의존하는 방법"이라는 점이다.** ACF(전역최대)는 두 세트 모두 lenient 100%이므로 항상 배수를 반환하는데, 교대형에서는 참 주기 lag의 자기상관이 배수보다 뚜렷이 높아 전역 최댓값이 참 주기에 놓이지만 등차 배치에서는 배수 lag의 자기상관이 참 주기와 거의 같아져 최댓값 위치가 노이즈로 결정된다(8.2.3절 3항). FFT는 교대형의 강한 2차 조화파가 등차 배치에는 없다(8.2.3절, 8.3절). 기하 2단계·ACF(첫국소최대)·직접위상은 패턴 구조보다 위상 정렬 자체에 의존하므로 세 방법 모두 부호가 유지된다. **어느 쪽이 실제인가**: 물리 기반 세트(대표 결과)에서 재면 기하 2단계는 ACF(전역최대)를 −18.8%p로, FFT를 −33.4%p로 유의하게 이긴다 — 이 두 쌍은 통제 세트 쪽 부호가 재현되었다.

3. **채점 기준에 따라 순위가 바뀐다(C4).** lenient 기준에서는 ACF(전역최대)가 100.0%로 1위가 되며, 기하 2단계와의 격차도 +32.6%p로 벌어져 유의해진다(Holm p = 1.46×10⁻⁵²)[로그 C:L305]. 이 방법이 실패할 때 참 주기의 **배수**를 반환하기 때문이다. **즉 "어느 방법이 나은가"는 채점 기준을 정하기 전에는 답할 수 없는 질문이다.**
4. **[개정 v6 — C14] P=6 취약성의 원인 정정.** v3~v5는 여기서 "대비를 맞춰도(통제 세트) 기하 2단계의 P=6 성공률이 0%로 떨어지므로 P=6 취약성은 난이도 탓이 아니라 **방법 자체의 성질**"이라고 결론지었다. 그러나 이 결론은 v4가 8.2.3절 4항을 신설하며 스스로 무너뜨렸다 — 통제 세트 P=6의 0%(반환값 {1:90})는 median H가 참 주기 N=6(0.588)을 N=1(0.988)보다 정확히 낮게 가리키는데도 "오름차순 첫 정합점" 규칙이 N=1을 먼저 채택하는, **통제 세트(등차 배치) 특유의 판정 규칙 인공물**이었다(8.2.3절 4항). "방법 자체의 성질"이 아니라 **"이 진단용 세트와 이 판정 규칙의 조합에서 나온 인공물"**로 정정한다. 다만 물리 기반 세트(대표 결과)에서도 기하 2단계·ACF(첫국소최대)는 P=6에서만 25~30%로 낮다(8.2.4절 주기별 표) — 이는 **통제 세트의 인공물과는 별개의, 아직 규명되지 않은 현상**이며 §9 남은 과제 (e)로 남긴다.

   > **[개정 v6 — 한 줄 참고]** 위 대비 지표 C의 값(예: P=4 high)은 절에 따라 **1.316(v3 정의, 미세 딜레이 제외) 또는 1.289(v4 정의, 미세 딜레이 포함, 8.2.4절부터 사용)** 로 다르게 인용되지만, 둘 다 **같은 패턴(P=4 high)을 가리키며 대소 관계·결론은 어느 정의로도 바뀌지 않는다**(정의의 전체 설명은 8.2.4절 도입부 M14 각주 참고).
...
6. **FFT는 "평가 불가"가 아니라 "설비 유형 의존"으로 정정한다 (v4, C7).** v3은 통제 세트 결과로 기하학의 열세는 "확인"하면서 FFT에 대해서는 "본 논문의 실험 설계로는 평가할 수 없다"고 적어, 같은 실험을 한쪽엔 채택하고 한쪽엔 실격시키는 이중 잣대라는 지적을 받았다. 8.2.3절의 위계 선언에 따라 두 방법 모두에 같은 잣대를 적용하면: 통제 세트의 결과는 방법을 가리지 않고 모두 조건부다. 물리 기반 세트(8.2.4절)에서 재면 **FFT는 짝수 주기(2매 세트) 14.3%, 홀수 주기(순차 stage) 71.1%, 전체 34.9%**다 — FFT의 성능은 "평가 불가"가 아니라 **설비 유형에 강하게 의존**한다. 다만 알림 응용(§1.2 목적 2)에서 FFT는 미탐율 100%로 사용 불가다(8.2.4절). 따라서 **FFT를 최종 권고 후보에서 조건부로 복귀시키되, 용도를 한정**한다: 순차 stage 라인의 1매당 시간 산출(목적 1)에는 쓸 수 있고, 알림(목적 2)에는 쓸 수 없다.
7. **산발적 검출 누락(결손)이 있으면 순위가 다시 한번, 이번엔 크게 뒤집힌다 (v4, 8.2.5절).** 이 설비에서 오검출(삽입)은 실재하지 않지만 검출 누락은 현실적 위협이다. 5% 미만의 산발적 결손만으로 클린 데이터 1위인 직접위상(일반화)은 39.7%로 추락하고, McNemar 대응표본 검정(집계, n=1,320)으로 확인하면 기하 2단계는 ACF(전역최대)와 통계적 동률(공동 1위)이 되며 직접위상(일반화)을 포함한 나머지 4개 방법을 전부 유의하게 이긴다. 원인은 알고리즘이 아니라 데이터 구조다 — 결손이 만드는 인덱스 밀림이 후보 주기 N의 약수와 우연히 일치할 확률(1/N)이 작은 N일수록 높으므로, 산발적 결손은 검출을 참 주기의 작은 약수 쪽으로 체계적으로 편향시킨다. 저대비 조건과 결손이 겹치면 6개 방법 전부가 예외 없이 0%로 무너지는 시나리오도 존재한다. **[개정 v6 — C15]** 다만 위 "공동 1위"는 rate=0.5/1/2/4%를 동일 가중으로 묶은 집계일 뿐이다 — rate별로 풀면 기하 2단계는 rate=0~0.5%에서 ACF(전역최대)를 **유의하게 이기고**(rate=0.5%에서 68.5%로 단독 1위), rate=1.0%는 통계적으로 갈리지 않으며, rate=2~4%에서만 ACF(전역최대)가 유의하게 이긴다(8.2.5절 rate별 McNemar). 상세는 8.2.5절과 8.4절 권고표 참고.

**[개정 v3] P=5가 쉬웠던 이유** — v2는 "P=5에서 두 방법이 100%로 동률"이라고만 적고 원인을 설명하지 않았다. 6.2절의 홀짝 구조 설명과 연결하면 분명하다. **짝수 주기는 짝수 후보 N과 위상 구조를 공유한다** — 참 주기 4인 데이터에서 후보 N=2는 phase0을 phase2와 비교하게 되므로, 두 값이 가까우면 틀린 N=2도 평탄해 보인다(6.2절). P=6도 마찬가지로 N=2, 3이 혼동원이 된다. 반면 **홀수 주기 P=5에는 그런 짝수 배수 혼동원이 N_max=20 안에 존재하지 않아**(약수가 1과 5뿐) 오탐 경로 자체가 없다. 즉 P=5의 100%는 방법의 우수성이 아니라 **혼동원 부재**의 결과다. 8.2.3절에서 대비를 맞춘 뒤에도 P=5만 100%로 남는 것이 이를 뒷받침한다. **[개정 v6]** 이 원리는 8.2.4절이 C 정의를 바꾸며(1.316→1.289, M14) 값이 미세하게 달라져도 그대로 성립한다 — 대소 관계와 혼동원 유무는 정의와 무관하다.

**따라서 개정된 권고는 데이터 신뢰도(검출 누락 위험)에 따라 갈린다** — 근거 세트는 **물리 기반 세트의 클린 조건(8.2.4절, 990회)과 결손 스트레스 조건(8.2.5절, 1,650회)** 두 가지이며, 원래 세트와 통제 세트는 진단 자료로 참조한다(8.2.3절 위계 선언).

| 용도 / 데이터 상태 | 권고 | 근거 |
|---|---|---|
| 1매당 시간 산출 (목적 1), **클린 로그** | **직접위상(일반화)** 92.1%(990회, 정상+드리프트+중간시프트 3조건 혼합) / 77.9%(330회, 정상 조건만) — **어느 세트로 봐도 1위** | 8.2.4절(990회), 8.2.5절 rate=0 기준선(330회, C9) |
| 1매당 시간 산출 (목적 1), **검출 누락 위험 있음** | **결손률을 모른다면 ACF(전역최대)** 62.0%(집계) — 기하 2단계(60.3%)와 집계 McNemar로는 통계적 동률(Holm p=0.147)이나 구현이 압도적으로 싸다(~10줄). **[개정 v6 — C15] 단, 이 권고는 조건부다**: rate별로 쪼개면 rate=0~0.5%에서는 **기하 2단계가 유의하게 우세**하고, rate=2~4%에서만 ACF(전역최대)가 유의하게 우세다(8.2.5절 rate별 McNemar). ACF(전역최대)를 "결손 대비용"으로 고르는 것은 **결손이 실제로, 그것도 rate≥2% 수준으로 있을 때만 정당화된다** — rate=0(클린)에서 이 방법은 6개 중 4위(45.5%)로 추락하므로, 결손률을 모르는 채 이 방법을 고르는 것은 "데이터 결함이 방법의 약점을 보정해 주기를 기대하는 것"과 같다. | 8.2.5절, 8.2.5절 rate별 층화(C15) |
| 설비 이상 알림 (목적 2), **클린 로그** | **직접위상(일반화)** (오경보 13.3%, 미탐 0.0%) | 8.2.4절 오경보/미탐 표 |
| 설비 이상 알림 (목적 2), **검출 누락 위험** | **미검증** — 8.2.5절은 시간 산출(목적 1) 성능만 측정했다 | 9절 남은 과제 |
| 사용 금지 (클린 기준) | **FFT** (미탐 100%), **ACF(전역최대)** (클린 알림에서 오경보 73.3%) | 8.2.4절 |

> **[개정 v5 — 표 스코프 명시 (M17)]** ACF(전역최대)가 두 번째 행에서는 권고되고 다섯 번째 행에서는 금지되는 것은 모순이 아니라 **목적(용도)이 다르기 때문**이다(위 표 아래 스코프 설명 — Change Log M17 참고). 이 방법은 항상 참 주기의 **배수**를 반환하는 경향이 있다(lenient 100%, 8.2.2절). 목적 1(시간 산출)에서는 배수든 참값이든 "위상이 정렬되는 윈도우"라는 목적에 크게 어긋나지 않아 결손 조건에서도 실용적이지만, 목적 2(알림)에서는 배수 반환이 곧 오진단(오경보)이므로(8.2.1절) 같은 성질이 치명적 결함이 된다. **같은 방법의 같은 성질이 목적에 따라 장점도 단점도 될 수 있다는 것이 이 표의 핵심 메시지다.**

**[개정 v6 — C15] "기하 2단계가 단독 1위인 경우는 없다"(v5)는 더 이상 정확하지 않다 — rate=0.5%에서는 기하 2단계가 68.5%로 단독 1위다(8.2.5절).** 클린 로그(집계 990회 또는 330회 기준선)에서는 여전히 직접위상(일반화)에 유의하게 밀리고(68.4% vs 92.1%/77.9%, 어느 쪽과 비교해도 유의), 결손 위험 조건의 **집계**(rate=0.5/1/2/4% 동일 가중)에서는 ACF(전역최대)와 McNemar 대응표본 검정으로 **통계적 동률**이다(차이 +1.7%p, Holm p=0.147)[8.2.5절]. 그러나 이 집계를 rate별로 풀면 기하 2단계는 rate=0~0.5%에서 **유의하게 이기고**, rate=2~4%에서만 유의하게 진다 — "동률"은 승–무–패가 평균으로 상쇄된 결과이지, 모든 조건에서 진짜 동률이라는 뜻이 아니다(8.2.5절 C15). **불리한 결과(클린 로그 3위, rate=2~4% 열세)를 숨기지 않은 것과 같은 기준으로, 이 유리한 결과(rate=0~0.5% 우세)도 보고한다.**

**그럼에도 "구현 비용이 결정적"이라는 결론 자체는 유지된다 — 다만 근거가 바뀐다.** 실제 결손률을 사전에 알 수 없다는 것이 이 권고 전체의 전제이므로(M19), rate별로 승패의 부호가 바뀌는 상황에서 "어느 쪽이 이긴다"고 사전에 단정할 수는 없다. 이 **부호 자체의 불확실성**이 구현이 압도적으로 싼 ACF(전역최대)를 기본값으로 미는 논거가 된다 — 원래 v5의 논거("성능이 같으니 싼 쪽을 쓴다")가 "성능의 우열이 조건에 따라 갈리고 그 조건을 사전에 모르니, 싼 쪽을 기본값으로 하되 실제 결손률을 알게 되면 표를 다시 참조한다"는 조건부 논거로 정교화된 것이다. **따라서 "표준 기법에 자리를 내주는 것이 합리적"이라는 결론은 유지되지만, 그 표준 기법이 무엇인지는 조건에 따라 직접위상(일반화)→ACF(전역최대)로 달라지고, 결손률이 낮다고 알려진 경우(rate≤0.5%)라면 오히려 기하 2단계가 성능상 우세하다는 것이 이번 개정에서 추가된 단서다.**

**기하학적 방법의 위치**: 클린 물리 기반 세트에서 6개 중 3위(68.4%)로, ACF(첫국소최대)(+13.3%p)와 직접위상(일반화)(+23.7%p)에 **유의하게 밀리고**, ACF(전역최대)·직접위상(2배)·FFT는 **유의하게 이긴다.** 결손 스트레스 조건에서는 위치가 뒤바뀐다 — 집계로는 ACF(전역최대)와 McNemar로 통계적 동률(공동 1위)이 되고, 클린 조건에서 자신을 이겼던 ACF(첫국소최대)와 직접위상(일반화)을 이번엔 **유의하게 이긴다**(각각 Holm p=8.48×10⁻²³, 2.28×10⁻⁴⁴). **[개정 v6 — C15] 이 집계를 rate별로 풀면 "동률"은 평균으로 상쇄된 결과다** — 기하 2단계는 rate=0~0.5%에서 ACF(전역최대)를 **유의하게 이기고**(rate=0.5%에서는 68.5%로 단독 1위), rate=2~4%에서만 유의하게 진다(8.2.5절). 다만 알림 적합성은 클린 조건에서 여전히 더 나쁘고(오경보 37.8%, 미탐 10.8%), 결손 조건에서의 알림 적합성은 측정하지 않았다(9절 남은 과제). **구현 비용을 근거로 한 권고("어느 조건에서도 이 복잡도를 정당화할 근거는 없다")는 실제 결손률을 사전에 모른다는 전제 위에서만 유지된다** — 승패의 부호 자체가 rate에 따라 갈리므로, 결손률을 모르는 채로는 구현이 압도적으로 싼 ACF(전역최대)를 기본값으로 두는 것이 합리적이지만, **결손률이 낮다고(rate≤0.5%) 알려진 조건이라면 기하 2단계가 오히려 성능상 우세하다.** v3까지의 "표준 기법에 밀린다"는 뭉뚱그린 서술은 **"클린 데이터에서는 두 방법에 밀리고 세 방법을 이기며, 결손 스트레스 조건에서는 rate가 낮을 때 가장 싼 표준 기법을 유의하게 이기고 rate가 높을 때만 유의하게 지며, 나머지 넷은 rate 무관하게 유의하게 이긴다"**로 정정된다.

다만 기하학적 접근이 무가치했던 것은 아니다. "세 점으로 삼각형을 만들어 납작한지 본다"는 그림은 문제를 처음 설명하고 직관을 잡는 데 유용했고, 그 직관에서 출발해 S의 스케일 종속성, κ의 과잉 정규화, R·H의 상관된 실패, median/mean의 강건성-검출력 트레이드오프, 그리고 마지막으로 **후보 집합 편향과 다중비교 문제, 산발적 결손에 대한 데이터 구조적 취약성**까지 하나씩 발견하고 고쳐나간 과정 자체가 통계적 사고를 기르는 학습 경로였다. 요컨대 기하학적 방법은 **탐구·교육 자료로는 의의가 있지만, 최종 배포 알고리즘으로는 (조건에 따라 다른) 표준 기법에 자리를 내주는 것이 합리적**이다.
...
**[개정 v5 — 대표 결과의 교체 (C10)]** v4는 8.2.3절에서 "물리 기반 세트가 권고의 주 근거이고, 원래 세트(540회)·통제 세트(270회)는 진단 자료로 참조한다"고 위계를 선언했다. 그런데 그 직후 §9 본문은 여전히 무효화된 540회 세트의 순위를 대표 결과로 제시하는 모순을 남겼다. 이번 개정에서 바로잡는다 — **아래 순서는 대표 결과(물리 기반 세트) → 결손 조건 → 방법론적 시행착오의 기록(원래·통제 세트)이다.**

**대표 결과: 설비 물리 기반 검증 (8.2.4절, 990회, 정상+드리프트+중간시프트)**

§1.1의 설비 구조(짝수 주기=2매 동시 생산, 홀수 주기=순차 stage)에서 직접 구성한 11패턴×3조건×30회 = 990회로 검증한 결과, 성공률(strict)은 **직접위상(일반화) 92.1% > ACF(첫국소최대) 81.7% > 기하 2단계 68.4% > ACF(전역최대) 49.6% > 직접위상(2배) 40.7% > FFT 34.9%**다. 대응표본 McNemar+Holm 검정 결과:

- 기하 2단계는 ACF(첫국소최대)(+13.3%p, Holm p=1.20×10⁻¹⁶)와 직접위상(일반화)(+23.7%p, Holm p=1.30×10⁻³⁹)에 **유의하게 열세**다.
- 기하 2단계는 ACF(전역최대)(−18.8%p, Holm p=7.85×10⁻¹⁶), 직접위상(2배)(−27.7%p), FFT(−33.4%p)에는 **유의하게 우세**다.
- **즉 6개 중 3위이며, 두 방법에 밀리고 세 방법을 이긴다** — "표준 기법에 밀린다"는 단정적 서술은 이렇게 정정된다.

이 결과가 물리적으로 실재하지 않는 v1~v3의 P=5 패턴(`10, 0.1, 9, 0.1, 9.5` — 홀수 주기인데 2매 동시 딜레이가 섞여 있어 순환 시 구조가 깨진다)을 대체해 처음으로 확보한, **실제 설비 구조에 대응하는** 검증 결과라는 점이 이전 세 판본과의 근본적 차이다.

**결손 조건 (8.2.5절, C8/C9 규명 후)**

같은 물리 기반 패턴에 산발적 검출 누락(rate<5%)을 섞으면 순위가 다시 뒤집힌다. 올바른 비교 기준선(정상 조건 rate=0, 330회)으로 확인하면 직접위상(일반화)은 77.9%→39.7%(−38.2%p)로 붕괴하고, 기하 2단계는 62.7%→60.3%(−2.4%p)로 거의 그대로다. 집계 McNemar 검정(n=1,320)으로 기하 2단계는 ACF(전역최대)와 통계적 동률(공동 1위)이 되며 나머지 넷을 유의하게 이긴다. 다만 **결손이 성능을 개선하는 사례가 11패턴 중 8개에서 관측**되었다 — clean 데이터에서 참 주기의 배수들이 median H·ACF 값에서 사실상 tie를 이루는데, 결손이 그 tie를 깨면서 가장 작은 배수만 남기 때문이며, 이는 8.2.5절 본편의 "결손이 작은 약수 쪽으로 편향시킨다"는 원리의 반대쪽 얼굴이다(코드 버그 아님, C8에서 규명). **[개정 v6 — C15] 이 "동률" 집계는 rate=0.5/1/2/4%를 동일 가중으로 묶은 값이며, rate별로 풀면 승–무–패가 갈린다** — 기하 2단계는 rate=0~0.5%에서 ACF(전역최대)를 유의하게 이기고(rate=0.5%에서 68.5%로 단독 1위), rate=1.0%는 통계적으로 갈리지 않으며, rate=2~4%에서만 ACF(전역최대)가 유의하게 이긴다(8.2.5절). 실제 결손률을 사전에 알 수 없다는 전제 위에서는, 구현 비용이 압도적으로 싼 ACF(전역최대)를 기본값으로 두는 것이 여전히 합리적이지만 — 결손률이 낮다고(rate≤0.5%) 알려진 경우라면 이 결과가 기하학적 방법의 채택 근거가 될 수 있다.

**방법론적 시행착오의 기록 (원래 세트 540회 / 통제 세트 270회)**

아래 두 세트는 **더 이상 권고의 근거가 아니며**, 검증 방법 자체가 개선되어 온 과정을 정직하게 남기기 위해 유지한다.

- **원래 세트(교대형, 540회)**: 성공률 직접위상(일반화) 91.9% > ACF(첫국소최대) 76.1% > ACF(전역최대) 70.2% > 기하 2단계 64.3% > 직접위상(2배) 28.9% > FFT 0%. **유효 범위: 짝수 주기(P=4, P=6)만.** 이 세트의 P=5 패턴은 물리적으로 성립하지 않는 설비였고(§1.1), 540회 중 180회(1/3)를 차지했다. 기하 2단계는 하필 그 무효 구간에서 100%를 받았으므로, 총계 64.3%는 부풀려져 있었다.
- **통제 세트(등차 배치, 270회)**: 대비 지표를 통제해 패턴 난이도 교란을 진단하기 위한 세트로, 6.2절의 홀짝 혼동원까지 함께 제거해 **실제 설비 어디에도 대응하지 않는다.** 진단 결과 기하 2단계의 열세는 대비를 맞춰도 사라지지 않고(+26.3%p, Holm p=3.54×10⁻¹⁴), FFT는 0%→100%로 뒤집혀 옥타브 오류가 패턴 구조의 산물임이 확인되었다.

McNemar 정확검정 + Holm 보정(v2가 쓴 Wilson 신뢰구간 겹침 판정은 대응표본에 부적절했다 — C3)과 strict/lenient 이중 채점(C4)은 세 세트 모두에 공통으로 적용된 방법론이다. lenient 기준에서는 배수 반환을 허용하므로 순위가 다시 바뀐다 — 예컨대 원래 세트에서 ACF(전역최대)는 70.2%→100.0%로 1위가 되는데, 이는 이 방법이 "주기를 못 찾는" 것이 아니라 "**최소 주기를 못 고르는**" 것이기 때문이다. 채점 기준을 명시하지 않은 성능 주장은 성립하지 않는다(C4).

**비정상성(드리프트) 취약성은 조건부다.** 사인 드리프트는 저대비 조건에서만 성능을 떨어뜨리고, 대비가 큰 조건에서는 오히려 80.0%→100.0%로 개선한다(원래 세트, 8.3.1절). v2가 불리한 사례만 인용해 "취약성 재확인"이라 적은 것은 정정되었다.

**FFT는 최종 권고에서 완전히 배제되지 않는다 — 설비 유형에 조건부로 복귀한다 (개정 v5, C11).** 통제 세트에서 FFT가 0%→100%로 뒤집힌 것은 옥타브 오류가 "큰 값/작은 값 교대" 패턴 구조의 산물임을 보였을 뿐, FFT 자체의 무능이 아니었다. 물리 기반 세트에서 재면 FFT는 짝수 주기(2매 세트) 14.3%, 홀수 주기(순차 stage) **71.1%**로, 성능이 설비 유형에 강하게 의존한다(8.2.4절). 따라서 **순차 stage 라인의 1매당 시간 산출(목적 1)에는 FFT를 조건부로 권고**하고, 옥타브 오류가 나는 2매 세트 라인과 항상 미탐인 알림 용도(목적 2)에서만 배제한다(8.4절 6항). v3~v4 초기의 "FFT를 최종 권고에서 제외한 것은 옳았다"는 무조건적 서술은 이렇게 정정된다.
...
(산발적 결손 조건의 상세 결과는 위 "결손 조건" 문단과 8.2.5절을 참고.)

또한 개정 과정에서 일곱 가지를 새로 규명했다. 첫째, **v1이 근거로 삼은 직접 위상비교 구현은 2의 거듭제곱 주기만 반환할 수 있어**, v1의 "6/6 성공" 결론은 P=4에 한정된 것이었다(8.1절). 둘째, **ACF의 P=6 실패는 지표가 아니라 판정 규칙 때문**이며, 규칙을 바꾸면 성능 순위가 뒤집힌다(8.3절). 셋째, **통제 세트에서 기하 2단계가 P=6에서 0%인 것도 같은 원인**이다 — median H는 참 주기 N=6(0.588)을 N=1(0.988)보다 정확히 낮게 가리키는데, "오름차순 첫 정합점" 규칙이 N=1을 먼저 채택한다(8.2.3절 4항). 넷째, **홀수 주기 P=5가 쉬웠던 것은 방법의 우수성이 아니라 짝수 배수 혼동원의 부재** 때문이다(8.4절). 다섯째, **같은 "약수로의 편향" 원리가 산발적 결손에도 적용된다** — 나눠떨어질 작은 약수가 없는 홀수(소수) 주기는 결손에도 강하다(8.2.5절). 여섯째, **그 편향 원리는 방향을 바꿔 "결손이 성능을 개선하는" 사례도 설명한다** — clean 데이터에서 참 주기의 배수들이 통계적으로 tie를 이루는 경우, 결손이 그 tie를 깨면서 가장 작은 배수만 남긴다(8.2.5절, C8). **일곱째, 그 "동률"은 rate를 뭉친 집계일 뿐이며 rate별로 풀면 승-무-패 경계가 rate=1.0% 부근에 있다** — rate=0~0.5%에서는 기하 2단계가 유의하게 우세하고 rate=2~4%에서만 ACF(전역최대)가 유의하게 우세하다(8.2.5절, C15). 일곱 가지 모두 같은 곳을 가리킨다 — **이 문제에서 비교의 단위는 "지표"가 아니라 "지표 + 판정 규칙 + 채점 기준 + 데이터 품질(과 그 품질의 정도)"이라는 평가 체계 전체**다.
...
**남은 과제**는 다음과 같다. (a) 7.3.1절의 절차에 따라 사내에서 실측 검증을 수행하고, 집계값 공개 또는 대리 데이터 공개 중 하나로 7.3절을 채우는 것(현재 미검증 상태), (b) FFT에 배음 보정(예: 조화적 곱 스펙트럼)을 적용해 교대 패턴·좁은 듀티비 패턴 모두에서 공정하게 재평가하는 것 — **8.2.4절의 물리 기반 세트(순차 stage 71.1%)가 FFT의 잠재력과 한계(P=5의 듀티비 문제)를 함께 보인 만큼 우선순위가 높아졌다**, (c) 판정 규칙 자체를 데이터에서 학습하거나 주기 후보별로 적응시키는 방법의 탐색, (d) Σ⁻ₙ(n) ≈ N·d 근사의 이론적 오차 상한 유도, (e) 물리 기반 세트에서도 기하 2단계·ACF(첫국소최대)가 P=6에서만 25~30%로 무너지는 원인 규명 — 통제 세트의 P=6 0%는 판정 규칙 문제로 규명되었으나(8.2.3절 4항), 교대형 P=6의 실패는 아직 미규명이다, (f) 산발적 결손 조건에서 설비 이상 알림(§1.2 목적 2, 오경보/미탐)을 재평가하는 것 — 8.2.5절은 시간 산출(목적 1) 성능만 측정했다, (g) P=4 카세트지연-low에서 기하 2단계가 완만히 열화되는 동안 나머지 다섯 방법이 급격한 절벽을 보이는 현상(M15, rate=0의 73.3%부터 이미 상대적으로 양호했다)의 원인 규명 — 더 넓은 rate·패턴 조합으로 이 절벽이 재현되는지 확인해야 한다, (h) FFT의 P=5 듀티비 가설을 펄스 폭을 체계적으로 바꾼 재실험으로 직접 검증하는 것 — 현재는 "정확히 2를 반환한다"는 관측과 sinc 이론으로부터의 정성적 설명에 그친다.
...
`[로그 C:Lxx-yy]` 형식으로 표기하였다.

> **[개정 v5 — 접두어 (M13)]** Appendix C(본 부록, v3 로그, 532행)·F(v4 로그, 359행)·
> G(결손 로그, 194행)의 행 번호 범위가 서로 겹쳐, 접두어 없는 `[로그 Lxx]`는 어느
> 부록을 가리키는지 모호했다. 본문의 모든 인용에 `C:`/`F:`/`G:` 접두어를 붙였다.
> 이 과정에서 실제로 잘못된 범위를 가리키던 인용 5곳(4.2절, 8.1절 2곳, 8.3절 2곳)도
> 함께 발견해 정정하였다 — 예: 8.1절의 직접 위상비교 후보 집합 인용은 `L45-53`으로
> 잘못 표기되어 있었으나 실제 해당 내용은 `C:L166-173`에 있다.
...
| `verification_tooth.py` | 산발적 결손(tooth loss) 강건성 실험(8.2.5절): 확률 기반 결손 주입, McNemar 대응표본 검정 |
| `verification_tooth_v2.py` | 4차 리뷰 대응: 코드 경로 동일성 점검(C8-1), 배수 tie 해소 메커니즘 진단(C8-2), rate=0 기준선 재계산(C9), 개선 사례 전수 보고(C8-4) |
| `verification_tooth_v3.py` | 5차 리뷰 대응(C15): rate별 층화 재실행(Appendix G/H와 동일 시드), rate별 McNemar+Holm(기하2단계 vs ACF(전역최대)) |
...
git checkout 1182ba05d7cc5ab404938ca021f36f4af4cc8586                                                # v5 판본
python verification_tooth_v2.py # Appendix H의 로그를 재생성 (verification_tooth_v2_log.txt)
```
...
v3 로그는 그대로 두고, v4에서 새로 산출한 것만 여기 싣는다. 본문 인용은 `F:Lxx-yy`
접두어로 Appendix C(`C:`)·G(`G:`)와 구분한다(M13).
...
## Appendix G. 산발적 결손 실험 로그 *(v4 신규)*

8.2.5절(산발적 결손 강건성)의 근거 로그다. `verification_v4.py`가 아니라 별도
스크립트 `verification_tooth.py`로 산출하였다. 본문 인용은 `G:Lxx-yy` 접두어로
Appendix C(`C:`)·F(`F:`)와 구분한다(M13). C8/C9/M15 관련 후속 진단은
`verification_tooth_v2.py`가 산출한 Appendix H에 별도로 싣는다.

**실행 방법**
```
python verification_tooth.py    # verification_tooth_log.txt 생성
```
- `find_n.py`·`verification_v2.py`·`verification_v3.py`·`verification_v4.py`(패턴 정의)의
  함수를 수정 없이 import한다
- 결손 위치는 각 tact마다 독립적으로 확률 rate로 판정하므로(고정 개수 아님), 실제 발생
  개수는 이항분포를 따르는 확률변수다 — 로그에 매 조합마다 실제 평균·최소·최대를 남겼다
- seed=6000 고정이므로 전체가 결정적으로 재현된다

**로그 구간 색인**

| 로그 행 | 내용 | 본문 위치 |
|---|---|---|
| L13-L31 | median H(N) 메커니즘 진단 (저대비/고대비, 결손 전후 비교) | 8.2.5 |
| L52-L106 | 11패턴 × 5rate × 30회 = 1,650회 원자료 | 8.2.5 |
| L111-L167 | 시나리오별 성공률 [Wilson 95% CI] | 8.2.5 |
| L169-L174 | 전체·설비유형별 총계 | 8.2.5 |
| L178-L189 | **기하2단계 vs 나머지 5개 방법 McNemar + Holm (대응표본)** | 8.2.5, 8.4, 9 |

**로그 전문**

```text
L1   ========================================================================
L2   산발적 결손(tooth loss) 강건성 실험 로그
L3   ========================================================================
L4   실무 확인 결과, 이 설비에서 오검출(스퓨리어스 삽입)은 실재하지 않는다.
L5   검출 누락(결손)만 현실적인 실패 모드이므로 삽입(insert) 시나리오는 제외한다.
L6
L7
L8   ========================================================================
L9   [진단] 산발적 결손이 median H(N) 순위를 뒤집는 메커니즘
L10  ========================================================================
L11  seed=6000, rate=0.5%, 참 주기 P=4 패턴 두 종류로 비교
L12
L13  --- 저대비 (10, 0.1, 9, 0.1), 결손 93개(0.62%) ---
L14      N |  clean median H |  glitched median H |      변화율
L15      2 |          0.0392 |             0.0591 |    1.51x
L16      3 |          9.3763 |             9.3222 |    0.99x
L17      4 |          0.0383 |             0.1017 |    2.65x
L18      5 |          9.3881 |             9.2914 |    0.99x
L19      6 |          0.0395 |             0.1877 |    4.76x
L20      7 |          9.3921 |             9.2656 |    0.99x
L21      8 |          0.0379 |             0.1971 |    5.20x
L22
L23  --- 고대비 (10, 0.1, 5, 0.1), 결손 93개(0.62%) ---
L24      N |  clean median H |  glitched median H |      변화율
L25      2 |          0.0844 |             1.9390 |   22.97x
L26      3 |          7.3344 |             7.2493 |    0.99x
L27      4 |          0.0364 |             0.0675 |    1.85x
L28      5 |          7.1080 |             7.0193 |    0.99x
L29      6 |          0.1770 |             2.2871 |   12.92x
L30      7 |          7.4094 |             7.2452 |    0.98x
L31      8 |          0.0360 |             0.1362 |    3.78x
L32
L33  대상 패턴 (11개, 8.2.4절 물리 기반 세트 전체, 정상 조건):
L34    P=2 2매세트-단일
L35    P=3 순차stage-high
L36    P=3 순차stage-low
L37    P=4 2매세트-high
L38    P=4 2매세트-low
L39    P=4 카세트지연-high
L40    P=4 카세트지연-low
L41    P=5 순차stage-high
L42    P=5 순차stage-low
L43    P=6 2매세트-high
L44    P=6 2매세트-low
L45
L46  ========================================================================
L47  [실험 I / 신규] 무작위 산발적 결손(tooth loss) (rate < 5%)
L48  ========================================================================
L49  각 tact 위치를 독립적으로 확률 rate로 판정해 결손시킨다(검출 누락).
L50  모든 rate는 5% 미만이며, 실제 발생 개수는 평균 rate*L 근방의 확률변수다.
L51
L52    [P=2 2매세트-단일 delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=13/30, ACF(첫국소최대)=22/30, ACF(전역최대)=12/30, FFT=30/30, 직접위상(2배)=29/30, 직접위상(일반화)=19/30
L53    [P=2 2매세트-단일 delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=27/30, 직접위상(일반화)=27/30
L54    [P=2 2매세트-단일 delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=24/30, 직접위상(일반화)=24/30
L55    [P=2 2매세트-단일 delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=22/30, 직접위상(일반화)=20/30
L56    [P=2 2매세트-단일 delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=18/30, 직접위상(일반화)=18/30
L57    [P=3 순차stage-high delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=17/30, ACF(첫국소최대)=17/30, ACF(전역최대)=5/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=25/30
L58    [P=3 순차stage-high delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=30/30
L59    [P=3 순차stage-high delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=23/30
L60    [P=3 순차stage-high delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=28/30
L61    [P=3 순차stage-high delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=18/30
L62    [P=3 순차stage-low delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=16/30, ACF(첫국소최대)=13/30, ACF(전역최대)=1/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=24/30
L63    [P=3 순차stage-low delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=18/30, ACF(첫국소최대)=28/30, ACF(전역최대)=7/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=17/30
L64    [P=3 순차stage-low delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=25/30, ACF(첫국소최대)=30/30, ACF(전역최대)=15/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=16/30
L65    [P=3 순차stage-low delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=26/30, ACF(첫국소최대)=30/30, ACF(전역최대)=22/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=15/30
L66    [P=3 순차stage-low delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=28/30, ACF(첫국소최대)=30/30, ACF(전역최대)=27/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=4/30
L67    [P=4 2매세트-high delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=23/30, ACF(첫국소최대)=30/30, ACF(전역최대)=16/30, FFT=0/30, 직접위상(2배)=25/30, 직접위상(일반화)=22/30
L68    [P=4 2매세트-high delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=25/30, 직접위상(일반화)=26/30
L69    [P=4 2매세트-high delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=20/30, 직접위상(일반화)=19/30
L70    [P=4 2매세트-high delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=30/30, ACF(첫국소최대)=3/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=6/30, 직접위상(일반화)=9/30
L71    [P=4 2매세트-high delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=17/30, ACF(첫국소최대)=0/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=9/30, 직접위상(일반화)=6/30
L72    [P=4 2매세트-low delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=13/30, ACF(첫국소최대)=30/30, ACF(전역최대)=17/30, FFT=0/30, 직접위상(2배)=25/30, 직접위상(일반화)=22/30
L73    [P=4 2매세트-low delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L74    [P=4 2매세트-low delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L75    [P=4 2매세트-low delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L76    [P=4 2매세트-low delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L77    [P=4 카세트지연-high delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=26/30, ACF(첫국소최대)=29/30, ACF(전역최대)=13/30, FFT=0/30, 직접위상(2배)=24/30, 직접위상(일반화)=22/30
L78    [P=4 카세트지연-high delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=30/30, ACF(첫국소최대)=3/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=18/30, 직접위상(일반화)=18/30
L79    [P=4 카세트지연-high delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=30/30, ACF(첫국소최대)=0/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=12/30, 직접위상(일반화)=8/30
L80    [P=4 카세트지연-high delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=30/30, ACF(첫국소최대)=0/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=2/30, 직접위상(일반화)=1/30
L81    [P=4 카세트지연-high delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=2/30, 직접위상(일반화)=1/30
L82    [P=4 카세트지연-low delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=22/30, ACF(첫국소최대)=30/30, ACF(전역최대)=17/30, FFT=0/30, 직접위상(2배)=24/30, 직접위상(일반화)=22/30
L83    [P=4 카세트지연-low delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=30/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L84    [P=4 카세트지연-low delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L85    [P=4 카세트지연-low delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L86    [P=4 카세트지연-low delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L87    [P=5 순차stage-high delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=13/30, FFT=20/30, 직접위상(2배)=0/30, 직접위상(일반화)=25/30
L88    [P=5 순차stage-high delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=27/30, 직접위상(2배)=0/30, 직접위상(일반화)=29/30
L89    [P=5 순차stage-high delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=27/30, 직접위상(2배)=0/30, 직접위상(일반화)=28/30
L90    [P=5 순차stage-high delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=26/30
L91    [P=5 순차stage-high delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=24/30
L92    [P=5 순차stage-low delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=29/30, ACF(첫국소최대)=30/30, ACF(전역최대)=10/30, FFT=21/30, 직접위상(2배)=0/30, 직접위상(일반화)=23/30
L93    [P=5 순차stage-low delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=28/30, ACF(첫국소최대)=30/30, ACF(전역최대)=15/30, FFT=28/30, 직접위상(2배)=0/30, 직접위상(일반화)=18/30
L94    [P=5 순차stage-low delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=29/30, ACF(첫국소최대)=30/30, ACF(전역최대)=17/30, FFT=23/30, 직접위상(2배)=0/30, 직접위상(일반화)=14/30
L95    [P=5 순차stage-low delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=29/30, ACF(첫국소최대)=30/30, ACF(전역최대)=25/30, FFT=27/30, 직접위상(2배)=0/30, 직접위상(일반화)=4/30
L96    [P=5 순차stage-low delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=26/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=21/30, 직접위상(2배)=0/30, 직접위상(일반화)=4/30
L97    [P=6 2매세트-high delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=12/30, ACF(첫국소최대)=10/30, ACF(전역최대)=21/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=27/30
L98    [P=6 2매세트-high delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=28/30
L99    [P=6 2매세트-high delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=13/30
L100   [P=6 2매세트-high delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=6/30
L101   [P=6 2매세트-high delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=2/30
L102   [P=6 2매세트-low delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=6/30, ACF(첫국소최대)=7/30, ACF(전역최대)=25/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=26/30
L103   [P=6 2매세트-low delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L104   [P=6 2매세트-low delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L105   [P=6 2매세트-low delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L106   [P=6 2매세트-low delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L107
L108 ========================================================================
L109 [실험 I 요약] 산발적 글리치 비율(rate)별 성공률(%) [Wilson 95% CI]
L110 ========================================================================
L111 시나리오                           |                기하2단계 |           ACF(첫국소최대) |            ACF(전역최대) |                  FFT |             직접위상(2배) |            직접위상(일반화)
L112 ------------------------------------------------------------------------------------------------------------------------------------------------------------------------
L113 P=2 2매세트-단일 delete r=0.0%      |    43.3 [27.4, 60.8] |    73.3 [55.6, 85.8] |    40.0 [24.6, 57.7] |   100.0 [88.6,100.0] |    96.7 [83.3, 99.4] |    63.3 [45.5, 78.1]
L114 P=2 2매세트-단일 delete r=0.5%      |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |    90.0 [74.4, 96.5] |    90.0 [74.4, 96.5]
L115 P=2 2매세트-단일 delete r=1.0%      |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |    80.0 [62.7, 90.5] |    80.0 [62.7, 90.5]
L116 P=2 2매세트-단일 delete r=2.0%      |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |    73.3 [55.6, 85.8] |    66.7 [48.8, 80.8]
L117 P=2 2매세트-단일 delete r=4.0%      |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |    60.0 [42.3, 75.4] |    60.0 [42.3, 75.4]
L118 P=3 순차stage-high delete r=0.0% |    56.7 [39.2, 72.6] |    56.7 [39.2, 72.6] |    16.7 [ 7.3, 33.6] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |    83.3 [66.4, 92.7]
L119 P=3 순차stage-high delete r=0.5% |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |   100.0 [88.6,100.0]
L120 P=3 순차stage-high delete r=1.0% |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |    76.7 [59.1, 88.2]
L121 P=3 순차stage-high delete r=2.0% |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |    93.3 [78.7, 98.2]
L122 P=3 순차stage-high delete r=4.0% |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |    60.0 [42.3, 75.4]
L123 P=3 순차stage-low delete r=0.0%  |    53.3 [36.1, 69.8] |    43.3 [27.4, 60.8] |     3.3 [ 0.6, 16.7] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |    80.0 [62.7, 90.5]
L124 P=3 순차stage-low delete r=0.5%  |    60.0 [42.3, 75.4] |    93.3 [78.7, 98.2] |    23.3 [11.8, 40.9] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |    56.7 [39.2, 72.6]
L125 P=3 순차stage-low delete r=1.0%  |    83.3 [66.4, 92.7] |   100.0 [88.6,100.0] |    50.0 [33.2, 66.8] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |    53.3 [36.1, 69.8]
L126 P=3 순차stage-low delete r=2.0%  |    86.7 [70.3, 94.7] |   100.0 [88.6,100.0] |    73.3 [55.6, 85.8] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |    50.0 [33.2, 66.8]
L127 P=3 순차stage-low delete r=4.0%  |    93.3 [78.7, 98.2] |   100.0 [88.6,100.0] |    90.0 [74.4, 96.5] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |    13.3 [ 5.3, 29.7]
L128 P=4 2매세트-high delete r=0.0%    |    76.7 [59.1, 88.2] |   100.0 [88.6,100.0] |    53.3 [36.1, 69.8] |     0.0 [ 0.0, 11.4] |    83.3 [66.4, 92.7] |    73.3 [55.6, 85.8]
L129 P=4 2매세트-high delete r=0.5%    |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |    83.3 [66.4, 92.7] |    86.7 [70.3, 94.7]
L130 P=4 2매세트-high delete r=1.0%    |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |    66.7 [48.8, 80.8] |    63.3 [45.5, 78.1]
L131 P=4 2매세트-high delete r=2.0%    |   100.0 [88.6,100.0] |    10.0 [ 3.5, 25.6] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |    20.0 [ 9.5, 37.3] |    30.0 [16.7, 47.9]
L132 P=4 2매세트-high delete r=4.0%    |    56.7 [39.2, 72.6] |     0.0 [ 0.0, 11.4] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |    30.0 [16.7, 47.9] |    20.0 [ 9.5, 37.3]
L133 P=4 2매세트-low delete r=0.0%     |    43.3 [27.4, 60.8] |   100.0 [88.6,100.0] |    56.7 [39.2, 72.6] |     0.0 [ 0.0, 11.4] |    83.3 [66.4, 92.7] |    73.3 [55.6, 85.8]
L134 P=4 2매세트-low delete r=0.5%     |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4]
L135 P=4 2매세트-low delete r=1.0%     |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4]
L136 P=4 2매세트-low delete r=2.0%     |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4]
L137 P=4 2매세트-low delete r=4.0%     |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4]
L138 P=4 카세트지연-high delete r=0.0%   |    86.7 [70.3, 94.7] |    96.7 [83.3, 99.4] |    43.3 [27.4, 60.8] |     0.0 [ 0.0, 11.4] |    80.0 [62.7, 90.5] |    73.3 [55.6, 85.8]
L139 P=4 카세트지연-high delete r=0.5%   |   100.0 [88.6,100.0] |    10.0 [ 3.5, 25.6] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |    60.0 [42.3, 75.4] |    60.0 [42.3, 75.4]
L140 P=4 카세트지연-high delete r=1.0%   |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |    40.0 [24.6, 57.7] |    26.7 [14.2, 44.4]
L141 P=4 카세트지연-high delete r=2.0%   |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |     6.7 [ 1.8, 21.3] |     3.3 [ 0.6, 16.7]
L142 P=4 카세트지연-high delete r=4.0%   |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     6.7 [ 1.8, 21.3] |     3.3 [ 0.6, 16.7]
L143 P=4 카세트지연-low delete r=0.0%    |    73.3 [55.6, 85.8] |   100.0 [88.6,100.0] |    56.7 [39.2, 72.6] |     0.0 [ 0.0, 11.4] |    80.0 [62.7, 90.5] |    73.3 [55.6, 85.8]
L144 P=4 카세트지연-low delete r=0.5%    |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4]
L145 P=4 카세트지연-low delete r=1.0%    |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4]
L146 P=4 카세트지연-low delete r=2.0%    |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4]
L147 P=4 카세트지연-low delete r=4.0%    |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4]
L148 P=5 순차stage-high delete r=0.0% |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |    43.3 [27.4, 60.8] |    66.7 [48.8, 80.8] |     0.0 [ 0.0, 11.4] |    83.3 [66.4, 92.7]
L149 P=5 순차stage-high delete r=0.5% |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |    90.0 [74.4, 96.5] |     0.0 [ 0.0, 11.4] |    96.7 [83.3, 99.4]
L150 P=5 순차stage-high delete r=1.0% |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |    90.0 [74.4, 96.5] |     0.0 [ 0.0, 11.4] |    93.3 [78.7, 98.2]
L151 P=5 순차stage-high delete r=2.0% |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |    86.7 [70.3, 94.7]
L152 P=5 순차stage-high delete r=4.0% |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |    80.0 [62.7, 90.5]
L153 P=5 순차stage-low delete r=0.0%  |    96.7 [83.3, 99.4] |   100.0 [88.6,100.0] |    33.3 [19.2, 51.2] |    70.0 [52.1, 83.3] |     0.0 [ 0.0, 11.4] |    76.7 [59.1, 88.2]
L154 P=5 순차stage-low delete r=0.5%  |    93.3 [78.7, 98.2] |   100.0 [88.6,100.0] |    50.0 [33.2, 66.8] |    93.3 [78.7, 98.2] |     0.0 [ 0.0, 11.4] |    60.0 [42.3, 75.4]
L155 P=5 순차stage-low delete r=1.0%  |    96.7 [83.3, 99.4] |   100.0 [88.6,100.0] |    56.7 [39.2, 72.6] |    76.7 [59.1, 88.2] |     0.0 [ 0.0, 11.4] |    46.7 [30.2, 63.9]
L156 P=5 순차stage-low delete r=2.0%  |    96.7 [83.3, 99.4] |   100.0 [88.6,100.0] |    83.3 [66.4, 92.7] |    90.0 [74.4, 96.5] |     0.0 [ 0.0, 11.4] |    13.3 [ 5.3, 29.7]
L157 P=5 순차stage-low delete r=4.0%  |    86.7 [70.3, 94.7] |   100.0 [88.6,100.0] |   100.0 [88.6,100.0] |    70.0 [52.1, 83.3] |     0.0 [ 0.0, 11.4] |    13.3 [ 5.3, 29.7]
L158 P=6 2매세트-high delete r=0.0%    |    40.0 [24.6, 57.7] |    33.3 [19.2, 51.2] |    70.0 [52.1, 83.3] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |    90.0 [74.4, 96.5]
L159 P=6 2매세트-high delete r=0.5%    |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |    93.3 [78.7, 98.2]
L160 P=6 2매세트-high delete r=1.0%    |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |    43.3 [27.4, 60.8]
L161 P=6 2매세트-high delete r=2.0%    |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |   100.0 [88.6,100.0] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |    20.0 [ 9.5, 37.3]
L162 P=6 2매세트-high delete r=4.0%    |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     6.7 [ 1.8, 21.3]
L163 P=6 2매세트-low delete r=0.0%     |    20.0 [ 9.5, 37.3] |    23.3 [11.8, 40.9] |    83.3 [66.4, 92.7] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |    86.7 [70.3, 94.7]
L164 P=6 2매세트-low delete r=0.5%     |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4]
L165 P=6 2매세트-low delete r=1.0%     |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4]
L166 P=6 2매세트-low delete r=2.0%     |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4]
L167 P=6 2매세트-low delete r=4.0%     |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4] |     0.0 [ 0.0, 11.4]
L168
L169 [전체 방법별 총계 — 모든 패턴·모든 rate>0 합산]
L170   기하2단계             :  796/1320 =  60.3% [57.6, 62.9]
L171   ACF(첫국소최대)        :  664/1320 =  50.3% [47.6, 53.0]
L172   ACF(전역최대)         :  818/1320 =  62.0% [59.3, 64.6]
L173   FFT               :  573/1320 =  43.4% [40.8, 46.1]
L174   직접위상(2배)          :  185/1320 =  14.0% [12.2, 16.0]
L175   직접위상(일반화)         :  524/1320 =  39.7% [37.1, 42.4]
L176
L177 ========================================================================
L178 [실험 I / McNemar] 기하2단계 vs 나머지 (결손 rate>0, 대응표본)
L179 ========================================================================
L180 n=1320 (모든 패턴 x rate>0 x 반복 합산)
L181
L182 비교(baseline vs B)   baseline만성공     B만성공   차이(B-baseline)     Holm p     판정
L183 ----------------------------------------------------------------------------
L184 ACF(첫국소최대)                  162       30           -10.0%p   8.48e-23     유의
L185 ACF(전역최대)                    94      116            +1.7%p   1.47e-01   n.s.
L186 FFT                         250       27           -16.9%p   8.09e-46     유의
L187 직접위상(2배)                    619        8           -46.3%p  1.03e-170     유의
L188 직접위상(일반화)                   339       67           -20.6%p   2.28e-44     유의
L189
L190 saved: image\이미지_10_산발적결손_강건성.png
L191
L192 ========================================================================
L193 검증 로그 종료
L194 ========================================================================
...
## Appendix H. C8/C9/M15 재검증 로그 *(v5 신규)*

8.2.5절의 이상 현상 규명(C8: 결손이 성능을 개선하는 8개 패턴), 올바른 비교 기준선
산출(C9: rate=0 자체 기준선), 단독 생존 사례 재해석(M15)의 근거 로그다.
`verification_tooth.py`가 아니라 후속 스크립트 `verification_tooth_v2.py`로 산출하였다.
본문 인용은 `H:Lxx-yy` 접두어로 Appendix C(`C:`)·F(`F:`)·G(`G:`)와 구분한다(M13).

**실행 방법**
```
python verification_tooth_v2.py    # verification_tooth_v2_log.txt 생성
```
- `find_n.py`·`verification_v2.py`·`verification_v3.py`·`verification_v4.py`(패턴 정의)·
  `verification_tooth.py`(결손 주입 함수)의 함수를 수정 없이 import한다
- seed=6000 고정이므로 전체가 결정적으로 재현된다. Appendix G의 rate>0 원자료와
  본 부록의 rate>0 원자료가 같은 시드로 재실행되어 수치가 일치함을 직접 확인하였다
- **버그 수정 이력**: 최초 실행에서 rate=0 기준선의 분모 계산에 오류가 있었다
  (반복 횟수를 매 시행마다 누적해 330 대신 9,900이 되는 오류). 분자(성공 횟수)는
  처음부터 정확했으므로 결론에는 영향이 없었다 — 62.7%(207/330)는 정정 전에도
  207/9900=2.1%로 잘못 표시되었을 뿐, 207이라는 원시 수치 자체는 항상 옳았다.
  아래 로그는 수정된 스크립트로 재실행한 결과다.

**로그 구간 색인**

| 로그 행 | 내용 | 본문 위치 |
|---|---|---|
| L6-L11 | **C8-1: rate=0/rate>0 코드 경로 동일성 점검 (버그 아님 확인)** | 8.2.5절 |
| L14-L52 | **C8-2: 배수 tie 해소 메커니즘 진단** (median H·ACF 곡선, 개별 시행 사례) | 8.2.5절 |
| L68-L91 | **C9: rate=0 자체 기준선 재계산 (330회)** | 8.2.5절, 9절, 초록 |
| L94-L153 | rate>0 재실행 원자료 (Appendix G와 동일 시드, 수치 일치 확인용) | 8.2.5절 |
| L156-L190 | **C8-4: 개선된 8개 패턴 전수 보고** | 8.2.5절 |

**로그 전문**

```text
L1   ========================================================================
L2   4차 리뷰 C8/C9/M15 대응 로그
L3   ========================================================================
L4
L5   ========================================================================
L6   [C8-1] rate=0 vs rate>0 코드 경로 동일성 점검
L7   ========================================================================
L8     글리치 0개로 뽑힌 시행: 177/200
L9     [PASS] 글리치가 0개일 때 (x,y)는 rate=0 경로의 (x0,y0)와 배열 단위로 완전히 동일하다.
L10    결론: rate=0과 rate>0은 같은 코드 경로를 공유하며 분기점은 오직
L11          '실제로 몇 개가 삭제되는가' 뿐이다 — 별도의 코드 버그가 아니다.
L12
L13  ========================================================================
L14  [C8-2] 메커니즘 진단: clean 데이터에서 참 주기의 배수들이 통계적으로 tie된다
L15  ========================================================================
L16  패턴: [10.0, 0.1] (P=2), seed=6000, 결손 93개(0.62%)
L17
L18    N |  clean median H |  glitched median H
L19    2 |          0.0383 |             0.0547
L20    3 |          9.8930 |             9.8419
L21    4 |          0.0385 |             0.1081
L22    5 |          9.9033 |             9.8208
...
L25    8 |          0.0387 |             0.2082
L26    9 |          9.9103 |             9.7497
L27   10 |          0.0372 |             0.2706
L28   11 |          9.9145 |             9.7121
L29   12 |          0.0381 |             0.3243
L30   13 |          9.9172 |             9.6699
L31   14 |          0.0384 |             0.3702
L32
L33  lag |    clean ACF |  glitched ACF
L34    1 |      -0.9653 |       -0.9536
L35    2 |      +0.9652 |       +0.9416
L36    3 |      -0.9652 |       -0.9289
L37    4 |      +0.9649 |       +0.9167
...
L41    8 |      +0.9652 |       +0.8718
L42    9 |      -0.9648 |       -0.8605
L43   10 |      +0.9648 |       +0.8498
L44   11 |      -0.9647 |       -0.8392
L45   12 |      +0.9642 |       +0.8279
L46   13 |      -0.9646 |       -0.8177
L47   14 |      +0.9643 |       +0.8069
L48
L49  clean: 모든 짝수 N(=참 주기의 배수)의 median H가 사실상 동일(tie) —
L50         '첫 국소최솟값' 규칙이 표본잡음으로 임의 선택해 결과가 사실상 무작위다.
L51  glitched: N이 커질수록 median H가 단조 증가 — 가장 작은 배수(참 주기)가
L52            유일한 최솟값으로 남는다. ACF도 동일한 패턴(단조 감소).
L53
L54  [개별 시행 사례] 같은 패턴, seed 10개, rate=0 vs rate=0.5%의 반환값
L55    seed |    글리치 |      기하2단계 clean->glitched |    ACF전역최대 clean->glitched
L56    6000 |     93 |         10 -> 2                |          8 -> 2         
L57    6001 |     88 |          4 -> 2                |          4 -> 2         
L58    6002 |     76 |          4 -> 2                |          2 -> 2         
L59    6003 |     85 |          4 -> 2                |          4 -> 2         
L60    6004 |     88 |          8 -> 2                |         14 -> 2         
L61    6005 |     78 |          2 -> 2                |          4 -> 2         
L62    6006 |     75 |          2 -> 2                |          4 -> 2         
L63    6007 |     88 |          4 -> 2                |          2 -> 2         
L64    6008 |     80 |          4 -> 2                |          2 -> 2         
L65    6009 |     71 |         12 -> 2                |          4 -> 2         
L66
L67  ========================================================================
L68  [C9] rate=0 자체 기준선 재계산 (11패턴 x 30회 = 330회, 정상 조건)
L69  ========================================================================
L70  8.2.4절(990회, 정상+드리프트+중간시프트 혼합)과 조건 구성이 다르므로,
L71  8.2.5절 자신의 rate=0 부분집합을 올바른 비교 기준선으로 삼는다.
L72
L73    [P=2 2매세트-단일 rate=0.0%] 기하2단계=13/30, ACF(첫국소최대)=22/30, ACF(전역최대)=12/30, FFT=30/30, 직접위상(2배)=29/30, 직접위상(일반화)=19/30
L74    [P=3 순차stage-high rate=0.0%] 기하2단계=17/30, ACF(첫국소최대)=17/30, ACF(전역최대)=5/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=25/30
L75    [P=3 순차stage-low rate=0.0%] 기하2단계=16/30, ACF(첫국소최대)=13/30, ACF(전역최대)=1/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=24/30
L76    [P=4 2매세트-high rate=0.0%] 기하2단계=23/30, ACF(첫국소최대)=30/30, ACF(전역최대)=16/30, FFT=0/30, 직접위상(2배)=25/30, 직접위상(일반화)=22/30
L77    [P=4 2매세트-low rate=0.0%] 기하2단계=13/30, ACF(첫국소최대)=30/30, ACF(전역최대)=17/30, FFT=0/30, 직접위상(2배)=25/30, 직접위상(일반화)=22/30
L78    [P=4 카세트지연-high rate=0.0%] 기하2단계=26/30, ACF(첫국소최대)=29/30, ACF(전역최대)=13/30, FFT=0/30, 직접위상(2배)=24/30, 직접위상(일반화)=22/30
L79    [P=4 카세트지연-low rate=0.0%] 기하2단계=22/30, ACF(첫국소최대)=30/30, ACF(전역최대)=17/30, FFT=0/30, 직접위상(2배)=24/30, 직접위상(일반화)=22/30
L80    [P=5 순차stage-high rate=0.0%] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=13/30, FFT=20/30, 직접위상(2배)=0/30, 직접위상(일반화)=25/30
L81    [P=5 순차stage-low rate=0.0%] 기하2단계=29/30, ACF(첫국소최대)=30/30, ACF(전역최대)=10/30, FFT=21/30, 직접위상(2배)=0/30, 직접위상(일반화)=23/30
L82    [P=6 2매세트-high rate=0.0%] 기하2단계=12/30, ACF(첫국소최대)=10/30, ACF(전역최대)=21/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=27/30
L83    [P=6 2매세트-low rate=0.0%] 기하2단계=6/30, ACF(첫국소최대)=7/30, ACF(전역최대)=25/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=26/30
L84
L85  [rate=0 기준선 총계] n=330
L86    기하2단계             :  207/330 =  62.7% [57.4, 67.8]
L87    ACF(첫국소최대)        :  248/330 =  75.2% [70.2, 79.5]
L88    ACF(전역최대)         :  150/330 =  45.5% [40.2, 50.8]
L89    FFT               :  131/330 =  39.7% [34.6, 45.1]
L90    직접위상(2배)          :  127/330 =  38.5% [33.4, 43.8]
L91    직접위상(일반화)         :  257/330 =  77.9% [73.1, 82.0]
L92
L93  ========================================================================
L94  [실험 I / 신규] 무작위 산발적 결손(tooth loss) (rate < 5%)
L95  ========================================================================
L96  각 tact 위치를 독립적으로 확률 rate로 판정해 결손시킨다(검출 누락).
L97  모든 rate는 5% 미만이며, 실제 발생 개수는 평균 rate*L 근방의 확률변수다.
L98
L99    [P=2 2매세트-단일 delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=13/30, ACF(첫국소최대)=22/30, ACF(전역최대)=12/30, FFT=30/30, 직접위상(2배)=29/30, 직접위상(일반화)=19/30
L100   [P=2 2매세트-단일 delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=27/30, 직접위상(일반화)=27/30
L101   [P=2 2매세트-단일 delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=24/30, 직접위상(일반화)=24/30
L102   [P=2 2매세트-단일 delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=22/30, 직접위상(일반화)=20/30
L103   [P=2 2매세트-단일 delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=18/30, 직접위상(일반화)=18/30
L104   [P=3 순차stage-high delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=17/30, ACF(첫국소최대)=17/30, ACF(전역최대)=5/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=25/30
L105   [P=3 순차stage-high delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=30/30
L106   [P=3 순차stage-high delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=23/30
L107   [P=3 순차stage-high delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=28/30
L108   [P=3 순차stage-high delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=18/30
L109   [P=3 순차stage-low delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=16/30, ACF(첫국소최대)=13/30, ACF(전역최대)=1/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=24/30
L110   [P=3 순차stage-low delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=18/30, ACF(첫국소최대)=28/30, ACF(전역최대)=7/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=17/30
L111   [P=3 순차stage-low delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=25/30, ACF(첫국소최대)=30/30, ACF(전역최대)=15/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=16/30
L112   [P=3 순차stage-low delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=26/30, ACF(첫국소최대)=30/30, ACF(전역최대)=22/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=15/30
L113   [P=3 순차stage-low delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=28/30, ACF(첫국소최대)=30/30, ACF(전역최대)=27/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=4/30
L114   [P=4 2매세트-high delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=23/30, ACF(첫국소최대)=30/30, ACF(전역최대)=16/30, FFT=0/30, 직접위상(2배)=25/30, 직접위상(일반화)=22/30
L115   [P=4 2매세트-high delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=25/30, 직접위상(일반화)=26/30
L116   [P=4 2매세트-high delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=20/30, 직접위상(일반화)=19/30
L117   [P=4 2매세트-high delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=30/30, ACF(첫국소최대)=3/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=6/30, 직접위상(일반화)=9/30
L118   [P=4 2매세트-high delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=17/30, ACF(첫국소최대)=0/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=9/30, 직접위상(일반화)=6/30
L119   [P=4 2매세트-low delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=13/30, ACF(첫국소최대)=30/30, ACF(전역최대)=17/30, FFT=0/30, 직접위상(2배)=25/30, 직접위상(일반화)=22/30
L120   [P=4 2매세트-low delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L121   [P=4 2매세트-low delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L122   [P=4 2매세트-low delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L123   [P=4 2매세트-low delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L124   [P=4 카세트지연-high delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=26/30, ACF(첫국소최대)=29/30, ACF(전역최대)=13/30, FFT=0/30, 직접위상(2배)=24/30, 직접위상(일반화)=22/30
L125   [P=4 카세트지연-high delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=30/30, ACF(첫국소최대)=3/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=18/30, 직접위상(일반화)=18/30
L126   [P=4 카세트지연-high delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=30/30, ACF(첫국소최대)=0/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=12/30, 직접위상(일반화)=8/30
L127   [P=4 카세트지연-high delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=30/30, ACF(첫국소최대)=0/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=2/30, 직접위상(일반화)=1/30
L128   [P=4 카세트지연-high delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=2/30, 직접위상(일반화)=1/30
L129   [P=4 카세트지연-low delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=22/30, ACF(첫국소최대)=30/30, ACF(전역최대)=17/30, FFT=0/30, 직접위상(2배)=24/30, 직접위상(일반화)=22/30
L130   [P=4 카세트지연-low delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=30/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L131   [P=4 카세트지연-low delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L132   [P=4 카세트지연-low delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L133   [P=4 카세트지연-low delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L134   [P=5 순차stage-high delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=13/30, FFT=20/30, 직접위상(2배)=0/30, 직접위상(일반화)=25/30
L135   [P=5 순차stage-high delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=27/30, 직접위상(2배)=0/30, 직접위상(일반화)=29/30
L136   [P=5 순차stage-high delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=27/30, 직접위상(2배)=0/30, 직접위상(일반화)=28/30
L137   [P=5 순차stage-high delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=26/30
L138   [P=5 순차stage-high delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=24/30
L139   [P=5 순차stage-low delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=29/30, ACF(첫국소최대)=30/30, ACF(전역최대)=10/30, FFT=21/30, 직접위상(2배)=0/30, 직접위상(일반화)=23/30
L140   [P=5 순차stage-low delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=28/30, ACF(첫국소최대)=30/30, ACF(전역최대)=15/30, FFT=28/30, 직접위상(2배)=0/30, 직접위상(일반화)=18/30
L141   [P=5 순차stage-low delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=29/30, ACF(첫국소최대)=30/30, ACF(전역최대)=17/30, FFT=23/30, 직접위상(2배)=0/30, 직접위상(일반화)=14/30
L142   [P=5 순차stage-low delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=29/30, ACF(첫국소최대)=30/30, ACF(전역최대)=25/30, FFT=27/30, 직접위상(2배)=0/30, 직접위상(일반화)=4/30
L143   [P=5 순차stage-low delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=26/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=21/30, 직접위상(2배)=0/30, 직접위상(일반화)=4/30
L144   [P=6 2매세트-high delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=12/30, ACF(첫국소최대)=10/30, ACF(전역최대)=21/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=27/30
L145   [P=6 2매세트-high delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=28/30
L146   [P=6 2매세트-high delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=13/30
L147   [P=6 2매세트-high delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=6/30
L148   [P=6 2매세트-high delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=2/30
L149   [P=6 2매세트-low delete rate= 0.0% (실제 평균 0.0개, 0~0)] 기하2단계=6/30, ACF(첫국소최대)=7/30, ACF(전역최대)=25/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=26/30
L150   [P=6 2매세트-low delete rate= 0.5% (실제 평균 77.6개, 62~93)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L151   [P=6 2매세트-low delete rate= 1.0% (실제 평균 152.6개, 124~174)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L152   [P=6 2매세트-low delete rate= 2.0% (실제 평균 303.4개, 273~333)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L153   [P=6 2매세트-low delete rate= 4.0% (실제 평균 606.3개, 567~648)] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L154
L155 ========================================================================
L156 [C8-4] rate=0 대비 rate>0(집계)에서 성공률이 개선된 사례 전수 보고
L157 ========================================================================
L158 유리한 방향이든 불리한 방향이든, 예상(단조 악화)과 반대로 움직인 결과를 전부 싣는다.
L159
L160 패턴                     | 방법                 |   rate=0 |  rate>0 평균 |       변화
L161 ------------------------------------------------------------------------------
L162 P=2 2매세트-단일            | 기하2단계              |    43.3% |     100.0% |   +56.7%p
L163 P=2 2매세트-단일            | ACF(첫국소최대)         |    73.3% |     100.0% |   +26.7%p
L164 P=2 2매세트-단일            | ACF(전역최대)          |    40.0% |     100.0% |   +60.0%p
L165 P=2 2매세트-단일            | 직접위상(일반화)          |    63.3% |      74.2% |   +10.8%p
L166 P=3 순차stage-high       | 기하2단계              |    56.7% |     100.0% |   +43.3%p
L167 P=3 순차stage-high       | ACF(첫국소최대)         |    56.7% |     100.0% |   +43.3%p
L168 P=3 순차stage-high       | ACF(전역최대)          |    16.7% |     100.0% |   +83.3%p
L169 P=3 순차stage-low        | 기하2단계              |    53.3% |      80.8% |   +27.5%p
L170 P=3 순차stage-low        | ACF(첫국소최대)         |    43.3% |      98.3% |   +55.0%p
L171 P=3 순차stage-low        | ACF(전역최대)          |     3.3% |      59.2% |   +55.8%p
L172 P=4 2매세트-high          | 기하2단계              |    76.7% |      89.2% |   +12.5%p
L173 P=4 2매세트-high          | ACF(전역최대)          |    53.3% |     100.0% |   +46.7%p
L174 P=4 카세트지연-high         | ACF(전역최대)          |    43.3% |      75.0% |   +31.7%p
L175 P=5 순차stage-high       | ACF(전역최대)          |    43.3% |     100.0% |   +56.7%p
L176 P=5 순차stage-high       | FFT                |    66.7% |      95.0% |   +28.3%p
L177 P=5 순차stage-high       | 직접위상(일반화)          |    83.3% |      89.2% |    +5.8%p
L178 P=5 순차stage-low        | ACF(전역최대)          |    33.3% |      72.5% |   +39.2%p
L179 P=5 순차stage-low        | FFT                |    70.0% |      82.5% |   +12.5%p
L180 P=6 2매세트-high          | ACF(전역최대)          |    70.0% |      75.0% |    +5.0%p
L181
L182 개선이 하나라도 관측된 패턴 수: 8/11
L183   P=2 2매세트-단일
L184   P=3 순차stage-high
L185   P=3 순차stage-low
L186   P=4 2매세트-high
L187   P=4 카세트지연-high
L188   P=5 순차stage-high
L189   P=5 순차stage-low
L190   P=6 2매세트-high
L191
L192 ========================================================================
L193 검증 로그 종료
...
## Appendix I. C15 rate별 층화 분석 로그 *(v6 신규)*

8.2.5절의 rate별 층화 분석(C15: 집계가 뭉갠 "약수 편향 vs 배수 tie 해소" 두 메커니즘을
rate별로 분리, rate=0.5%에서 기하 2단계 단독 1위 확인, rate별 McNemar로 승-무-패 경계
확정)의 근거 로그다. `verification_tooth_v3.py`로 산출하였다. 본문 인용은 `I:Lxx-yy`
접두어로 Appendix C(`C:`)·F(`F:`)·G(`G:`)·H(`H:`)와 구분한다(M13 관례를 따름).

**실행 방법**
```
python verification_tooth_v3.py    # verification_tooth_v3_log.txt 생성
```
- `find_n.py`·`verification_v2.py`·`verification_v3.py`(패턴 정의·McNemar용 공통 함수)·
  `verification_tooth.py`(결손 주입 함수·물리 기반 패턴)의 함수를 수정 없이 import한다
- seed=6000 고정이므로 전체가 결정적으로 재현된다. Appendix G/H의 rate>0 원자료와
  본 부록의 rate>0 원자료가 같은 시드로 재실행되어 집계 성공률이 완전히 일치함을
  직접 확인하였다(예: rate=0 기준선 62.7%/75.2%/45.5%/39.7%/38.5%/77.9%는 Appendix H의
  C9 재계산과 소수점까지 일치한다)
- rate마다 대응표본(paired) 성공/실패 벡터를 별도로 저장해, Appendix G/H에는 없던
  **rate별 McNemar 검정**(기하2단계 vs ACF(전역최대), 5개 rate에 Holm-Bonferroni 보정)을
  신규로 산출한다
...
| 로그 행 | 내용 | 본문 위치 |
|---|---|---|
| L11-L65 | 11패턴×5rate×30회 원자료 (Appendix G/H와 동일 시드) | 8.2.5절 |
| L70-L76 | **C15-1: rate별 층화 성공률 표** (330회/rate) | 8.2.5절, 8.4절 권고표 |
| L78-L83 | rate별 전체 순위 (내림차순) | 8.2.5절 |
| L86-L89 | 기하2단계·ACF(전역최대) rate별 추이, 단조 증가 확인, rate=0.5% 단독 1위 확인 | 8.2.5절 |
| L92-L103 | **C15-5: rate별 McNemar 검정** (기하2단계 vs ACF(전역최대), Holm 보정) | 8.2.5절, 8.4절 권고표 |

**로그 전문**

```text
L1   ========================================================================
L2   5차 리뷰 C15 대응 로그 (rate별 층화 분석)
L3   ========================================================================
L4
L5   ========================================================================
L6   [실험 J / C15] rate별 층화 재실행 (기존 Appendix G와 동일 시드)
L7   ========================================================================
L8   Appendix G와 완전히 동일한 조건·시드로 재실행한다 — 집계 성공률이 일치하는지
L9   확인하고, 이번에는 rate마다 대응표본(paired) 벡터를 별도로 저장한다.
L10
L11    [P=2 2매세트-단일 rate= 0.0%] 기하2단계=13/30, ACF(첫국소최대)=22/30, ACF(전역최대)=12/30, FFT=30/30, 직접위상(2배)=29/30, 직접위상(일반화)=19/30
L12    [P=2 2매세트-단일 rate= 0.5%] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=27/30, 직접위상(일반화)=27/30
L13    [P=2 2매세트-단일 rate= 1.0%] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=24/30, 직접위상(일반화)=24/30
L14    [P=2 2매세트-단일 rate= 2.0%] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=22/30, 직접위상(일반화)=20/30
L15    [P=2 2매세트-단일 rate= 4.0%] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=18/30, 직접위상(일반화)=18/30
L16    [P=3 순차stage-high rate= 0.0%] 기하2단계=17/30, ACF(첫국소최대)=17/30, ACF(전역최대)=5/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=25/30
L17    [P=3 순차stage-high rate= 0.5%] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=30/30
L18    [P=3 순차stage-high rate= 1.0%] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=23/30
L19    [P=3 순차stage-high rate= 2.0%] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=28/30
L20    [P=3 순차stage-high rate= 4.0%] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=18/30
L21    [P=3 순차stage-low rate= 0.0%] 기하2단계=16/30, ACF(첫국소최대)=13/30, ACF(전역최대)=1/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=24/30
L22    [P=3 순차stage-low rate= 0.5%] 기하2단계=18/30, ACF(첫국소최대)=28/30, ACF(전역최대)=7/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=17/30
L23    [P=3 순차stage-low rate= 1.0%] 기하2단계=25/30, ACF(첫국소최대)=30/30, ACF(전역최대)=15/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=16/30
L24    [P=3 순차stage-low rate= 2.0%] 기하2단계=26/30, ACF(첫국소최대)=30/30, ACF(전역최대)=22/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=15/30
L25    [P=3 순차stage-low rate= 4.0%] 기하2단계=28/30, ACF(첫국소최대)=30/30, ACF(전역최대)=27/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=4/30
L26    [P=4 2매세트-high rate= 0.0%] 기하2단계=23/30, ACF(첫국소최대)=30/30, ACF(전역최대)=16/30, FFT=0/30, 직접위상(2배)=25/30, 직접위상(일반화)=22/30
L27    [P=4 2매세트-high rate= 0.5%] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=25/30, 직접위상(일반화)=26/30
L28    [P=4 2매세트-high rate= 1.0%] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=20/30, 직접위상(일반화)=19/30
L29    [P=4 2매세트-high rate= 2.0%] 기하2단계=30/30, ACF(첫국소최대)=3/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=6/30, 직접위상(일반화)=9/30
L30    [P=4 2매세트-high rate= 4.0%] 기하2단계=17/30, ACF(첫국소최대)=0/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=9/30, 직접위상(일반화)=6/30
L31    [P=4 2매세트-low rate= 0.0%] 기하2단계=13/30, ACF(첫국소최대)=30/30, ACF(전역최대)=17/30, FFT=0/30, 직접위상(2배)=25/30, 직접위상(일반화)=22/30
L32    [P=4 2매세트-low rate= 0.5%] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L33    [P=4 2매세트-low rate= 1.0%] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L34    [P=4 2매세트-low rate= 2.0%] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L35    [P=4 2매세트-low rate= 4.0%] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L36    [P=4 카세트지연-high rate= 0.0%] 기하2단계=26/30, ACF(첫국소최대)=29/30, ACF(전역최대)=13/30, FFT=0/30, 직접위상(2배)=24/30, 직접위상(일반화)=22/30
L37    [P=4 카세트지연-high rate= 0.5%] 기하2단계=30/30, ACF(첫국소최대)=3/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=18/30, 직접위상(일반화)=18/30
L38    [P=4 카세트지연-high rate= 1.0%] 기하2단계=30/30, ACF(첫국소최대)=0/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=12/30, 직접위상(일반화)=8/30
L39    [P=4 카세트지연-high rate= 2.0%] 기하2단계=30/30, ACF(첫국소최대)=0/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=2/30, 직접위상(일반화)=1/30
L40    [P=4 카세트지연-high rate= 4.0%] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=2/30, 직접위상(일반화)=1/30
L41    [P=4 카세트지연-low rate= 0.0%] 기하2단계=22/30, ACF(첫국소최대)=30/30, ACF(전역최대)=17/30, FFT=0/30, 직접위상(2배)=24/30, 직접위상(일반화)=22/30
L42    [P=4 카세트지연-low rate= 0.5%] 기하2단계=30/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L43    [P=4 카세트지연-low rate= 1.0%] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L44    [P=4 카세트지연-low rate= 2.0%] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L45    [P=4 카세트지연-low rate= 4.0%] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L46    [P=5 순차stage-high rate= 0.0%] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=13/30, FFT=20/30, 직접위상(2배)=0/30, 직접위상(일반화)=25/30
L47    [P=5 순차stage-high rate= 0.5%] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=27/30, 직접위상(2배)=0/30, 직접위상(일반화)=29/30
L48    [P=5 순차stage-high rate= 1.0%] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=27/30, 직접위상(2배)=0/30, 직접위상(일반화)=28/30
L49    [P=5 순차stage-high rate= 2.0%] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=26/30
L50    [P=5 순차stage-high rate= 4.0%] 기하2단계=30/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=30/30, 직접위상(2배)=0/30, 직접위상(일반화)=24/30
L51    [P=5 순차stage-low rate= 0.0%] 기하2단계=29/30, ACF(첫국소최대)=30/30, ACF(전역최대)=10/30, FFT=21/30, 직접위상(2배)=0/30, 직접위상(일반화)=23/30
L52    [P=5 순차stage-low rate= 0.5%] 기하2단계=28/30, ACF(첫국소최대)=30/30, ACF(전역최대)=15/30, FFT=28/30, 직접위상(2배)=0/30, 직접위상(일반화)=18/30
L53    [P=5 순차stage-low rate= 1.0%] 기하2단계=29/30, ACF(첫국소최대)=30/30, ACF(전역최대)=17/30, FFT=23/30, 직접위상(2배)=0/30, 직접위상(일반화)=14/30
L54    [P=5 순차stage-low rate= 2.0%] 기하2단계=29/30, ACF(첫국소최대)=30/30, ACF(전역최대)=25/30, FFT=27/30, 직접위상(2배)=0/30, 직접위상(일반화)=4/30
L55    [P=5 순차stage-low rate= 4.0%] 기하2단계=26/30, ACF(첫국소최대)=30/30, ACF(전역최대)=30/30, FFT=21/30, 직접위상(2배)=0/30, 직접위상(일반화)=4/30
L56    [P=6 2매세트-high rate= 0.0%] 기하2단계=12/30, ACF(첫국소최대)=10/30, ACF(전역최대)=21/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=27/30
L57    [P=6 2매세트-high rate= 0.5%] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=28/30
L58    [P=6 2매세트-high rate= 1.0%] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=13/30
L59    [P=6 2매세트-high rate= 2.0%] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=30/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=6/30
L60    [P=6 2매세트-high rate= 4.0%] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=2/30
L61    [P=6 2매세트-low rate= 0.0%] 기하2단계=6/30, ACF(첫국소최대)=7/30, ACF(전역최대)=25/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=26/30
L62    [P=6 2매세트-low rate= 0.5%] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L63    [P=6 2매세트-low rate= 1.0%] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L64    [P=6 2매세트-low rate= 2.0%] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L65    [P=6 2매세트-low rate= 4.0%] 기하2단계=0/30, ACF(첫국소최대)=0/30, ACF(전역최대)=0/30, FFT=0/30, 직접위상(2배)=0/30, 직접위상(일반화)=0/30
L66
L67  ========================================================================
L68  [C15-1] rate별 층화 성공률 표 (11패턴 x 30회 = 330회/rate)
L69  ========================================================================
L70    rate |              기하2단계 |         ACF(첫국소최대) |          ACF(전역최대) |                FFT |           직접위상(2배) |          직접위상(일반화) | 순위(1위)
L71  ---------------------------------------------------------------------------------------------------------------------------------------------
L72    0.0% |  62.7 [57.4, 67.8] |  75.2 [70.2, 79.5] |  45.5 [40.2, 50.8] |  39.7 [34.6, 45.1] |  38.5 [33.4, 43.8] |  77.9 [73.1, 82.0] | 직접위상(일반화)
L73    0.5% |  68.5 [63.3, 73.3] |  54.8 [49.5, 60.1] |  61.2 [55.9, 66.3] |  43.9 [38.7, 49.3] |  21.2 [17.1, 25.9] |  58.5 [53.1, 63.7] | 기하2단계
L74    1.0% |  61.8 [56.5, 66.9] |  54.5 [49.2, 59.8] |  64.2 [58.9, 69.2] |  42.4 [37.2, 47.8] |  17.0 [13.3, 21.4] |  43.9 [38.7, 49.3] | ACF(전역최대)
L75    2.0% |  62.1 [56.8, 67.2] |  46.4 [41.1, 51.8] |  68.8 [63.6, 73.5] |  44.5 [39.3, 49.9] |   9.1 [ 6.4, 12.7] |  33.0 [28.2, 38.3] | ACF(전역최대)
L76    4.0% |  48.8 [43.4, 54.2] |  45.5 [40.2, 50.8] |  53.6 [48.2, 58.9] |  42.7 [37.5, 48.1] |   8.8 [ 6.2, 12.3] |  23.3 [19.1, 28.2] | ACF(전역최대)
L77
L78  [rate별 전체 순위 (내림차순)]
L79    rate= 0.0%: 직접위상(일반화) > ACF(첫국소최대) > 기하2단계 > ACF(전역최대) > FFT > 직접위상(2배)
L80    rate= 0.5%: 기하2단계 > ACF(전역최대) > 직접위상(일반화) > ACF(첫국소최대) > FFT > 직접위상(2배)
L81    rate= 1.0%: ACF(전역최대) > 기하2단계 > ACF(첫국소최대) > 직접위상(일반화) > FFT > 직접위상(2배)
L82    rate= 2.0%: ACF(전역최대) > 기하2단계 > ACF(첫국소최대) > FFT > 직접위상(일반화) > 직접위상(2배)
L83    rate= 4.0%: ACF(전역최대) > 기하2단계 > ACF(첫국소최대) > FFT > 직접위상(일반화) > 직접위상(2배)
L84
L85  [특기 사항]
L86    기하2단계 rate별 추이: ['62.7', '68.5', '61.8', '62.1', '48.8']
L87    ACF(전역최대) rate별 추이: ['45.5', '61.2', '64.2', '68.8', '53.6']
L88    ACF(전역최대)가 rate=0~2%에서 단조 증가하는가: True
L89    rate=0.5%에서의 단독 1위: 기하2단계 (68.5%)
L90
L91  ========================================================================
L92  [C15-5] rate별 McNemar: 기하2단계 vs ACF(전역최대)
L93  ========================================================================
L94  각 rate 내에서 대응표본(n=330)으로 검정한다. 5개 rate에 걸친 다중비교이므로
L95  Holm-Bonferroni를 적용한다.
L96
L97    rate |    a만성공 |    b만성공 |    차이(b-a) |               95% CI |     Holm p | 판정
L98  ------------------------------------------------------------------------------------------
L99    0.0% |     118 |      61 |     -17.3%p | [  -25.2,   -9.7]%p |   1.23e-04 | 유의 (기하2단계 우세)
L100   0.5% |      58 |      34 |      -7.3%p | [  -13.0,   -1.5]%p |   3.20e-02 | 유의 (기하2단계 우세)
L101   1.0% |      24 |      32 |      +2.4%p | [   -2.1,   +7.0]%p |   3.50e-01 | n.s. (동률)
L102   2.0% |      11 |      33 |      +6.7%p | [   +2.7,  +10.6]%p |   3.78e-03 | 유의 (ACF(전역최대) 우세)
L103   4.0% |       1 |      17 |      +4.8%p | [   +2.4,   +7.3]%p |   5.80e-04 | 유의 (ACF(전역최대) 우세)
L104
L105 saved: image\이미지_11_rate별_층화_성공률.png
L106
L107 ========================================================================
L108 검증 로그 종료
L109 ========================================================================
...
## Appendix D. 리뷰 대응 체크리스트 *(v6에서 다섯 라운드 통합)*

### 5차 리뷰 (v5 → v6) 대응

| 항목 | 요구사항 | 조치 | 반영 위치 |
|---|---|---|---|
| **C12** | 8.4절 대표 결과를 물리 기반 세트로 교체 | ✅ 완료 (68.4%, 6개 중 3위 — §9와 동일 수치) | 8.4절 첫 항목 |
| **C12** | 540회 McNemar표·항목을 "방법론적 시행착오의 기록"으로 재구성 | ✅ 완료 (§9와 동일한 구조, 항목별 유효 범위 명시) | 8.4절 |
| **C13** | 8.3절의 "FFT 최종 권고에서 제외" 문장 교체 | ✅ 완료 (8.4절 6항 조건부 복귀 결론으로 통일) | 8.3절 |
| **C13** | 6개 키워드 전수 점검(최종 권고에서 제외/평가할 수 없다/유의하지 않다/64.3%/91.9%/방법 자체의 성질) | ✅ 완료 — 8.4절이 여전히 540회(64.3%)를 헤드라인으로 쓰던 것(1항)과 "방법 자체의 성질"(4항)에서 발견·정정, 5항 등 나머지 64.3%/91.9% 인용은 이미 "유효 범위"가 명시된 역사적 수치라 정합 | 8.2.1, 8.2.2, 8.3, 8.3.1, 8.4 |
| **C14** | 8.4절 4항 "방법 자체의 성질"을 8.2.3절 4항 결론으로 정정 | ✅ 완료 (통제 세트 판정 규칙 인공물로 정정) | 8.4절 |
| **C14** | 물리 기반 세트 자체 P=6 약점을 별개 미규명으로 분리 | ✅ 완료 | 8.4절, §9 남은 과제 (e) |
| **C15** | rate별 층화 성공률·순위 표 추가 | ✅ 완료 (Appendix G/H와 시드 일치 확인) | 8.2.5절, Appendix I |
| **C15** | rate=0.5% 기하2단계 단독 1위 명시 | ✅ 완료 — 불리한 결과와 같은 기준으로 보고 | 8.2.5절, 초록, 8.4절 |
| **C15** | ACF(전역최대) 단조 개선(rate 0~2%) 명시 | ✅ 완료 (45.5→61.2→64.2→68.8, rate=4%에서 꺾임) | 8.2.5절 |
| **C15** | rate별 McNemar(기하2단계 vs ACF전역최대) | ✅ 완료 — 승(0~0.5%)-무(1.0%)-패(2~4%) 경계 확정 | 8.2.5절, Appendix I:L92-103 |
| **C15** | 권고표에 조건 의존성 경고 추가 | ✅ 완료 ("결손률을 안다는 전제에서만 성립") | 8.4절 권고표 |
| **M18** | 권고표 클린 행에 77.9%(rate=0, 330회) 병기 | ✅ 완료 (92.1%와 세트 구분 명시) | 8.4절 권고표 |
| **M19** | rate 동일 가중이 임의적 가정임을 명시 | ✅ 완료 (실측 결손률 확보 시 해당 rate 행 참조로 대체 권고) | 8.2.5절 한계 박스 |
| **M20** | Change Log/Appendix D의 M17 "열 추가" 표기 정정 | ✅ 완료 ("표 아래 각주"로 정정) | Change Log v4→v5 이력, 본 표 4차 리뷰 M17 행 |
| Minor | 8.2.5절 소제목 "990회 대비"→"rate=0 기준선 대비" | ✅ 완료 | 8.2.5절 |
| Minor | FFT 듀티비 가설 서술 강도 완화(단정형→가설형) | ✅ 완료 (§9 (h)와 정합) | 8.2.4절 |
| Minor | C 정의(1.316/1.289) 동일 패턴 교차 참조 | ✅ 완료 | 8.4절 4항 옆 |
| Minor | 부록 색인 추가 | ✅ 완료 (문서 서두, 알파벳 순) | 문서 서두 |

**5차 리뷰 미이행 항목**: 없음. M1/M7(실측 데이터 집계)은 이번 라운드 범위에서 명시적으로
제외되었으며(사용자 지시), 7.3절·7.3.1절과 Appendix D의 해당 상태 표기는 그대로 유지했다 —
"추후 보강 예정"이라는 기존 사유를 완화하지 않았다.

**방법론 노트 — 절 간 정합성 점검 절차 (재발 방지)**

같은 유형의 전파 누락이 두 번 반복되었다 — v4에서는 C10·C11의 정정이 §9에만 적용되고
8.3·8.4절이 뒤처졌고(당시엔 발견되지 못함), v5에서는 C9의 정정이 8.2.5절에만 적용되고
8.4절 권고표는 그대로였다(이번 5차 리뷰 C12~C14가 지적). 두 번 반복된 이상 리뷰에 의존해
발견되기를 기다리기보다, 개정 작업 자체에 다음 절차를 강제한다.

1. **헤드라인 숫자·결론을 하나라도 바꿀 때는, 그 숫자·결론 문자열을 문서 전체에서
   grep해 등장하는 모든 절을 목록화**한 뒤에만 수정을 시작한다(예: "64.3%", "91.9%",
   "방법 자체의 성질" 같은 검색 가능한 리터럴을 우선한다).
2. **"대표/헤드라인" 결과를 바꾸는 경우, 초록·8.2.x·8.4·§9·Appendix D를 하나의 원자적
   집합으로 함께 갱신**한다 — 일부만 갱신하고 나머지를 "다음 라운드에서"로 미루지 않는다.
3. 개정을 마치기 전 **문서 전체에서 정체(stale) 키워드를 마지막으로 한 번 더 검색**한다:
   "최종 권고에서 제외", "평가할 수 없다", "방법 자체의 성질", 그리고 이번에 폐기한
   헤드라인 숫자 전부.
4. **Change Log가 주장하는 조치와 실제 문서 상태를 마지막에 대조**한다(예: M20 — Change
   Log가 "열 추가"라 적었는데 실제로는 각주였던 경우를 정정). Change Log 항목 하나마다
   본문에서 실제로 그렇게 되어 있는지 확인하지 않은 채 통과시키지 않는다.

### 4차 리뷰 (v4 → v5) 대응

| 항목 | 요구사항 | 조치 | 반영 위치 |
|---|---|---|---|
| **C8** | rate=0/rate>0 코드 경로 동일성 확인 | ✅ 완료 (버그 아님 — 배수 tie 해소 현상) | 8.2.5절, Appendix H:L6-11 |
| **C8** | 개선된 8개 패턴 전수 보고 | ✅ 완료 (8/11 패턴) | 8.2.5절 표, Appendix H:L156-190 |
| **C8** | 재발 방지 절차를 방법론 노트로 남김 | ✅ 완료 | 본 부록 하단 "방법론 노트" |
| **C9** | rate=0 자체 기준선(330회) 표 추가 | ✅ 완료 | 8.2.5절, Appendix H:L67-91 |
| **C9** | 순위 반전을 올바른 기준선으로 재서술 | ✅ 완료 (77.9%→39.7%, 62.7%→60.3%) | 8.2.5절, 9절, 초록 |
| **C9** | 8.2.4/8.2.5 조건 구성 차이 명시 | ✅ 완료 | 8.2.5절 한계 박스(M16과 통합) |
| **C10** | §9 첫 불릿을 물리 기반 세트로 교체 | ✅ 완료 | 9절, 초록 |
| **C10** | 540·270회 세트를 방법론 이력으로 재배치 | ✅ 완료 | 9절 "방법론적 시행착오의 기록" |
| **C11** | FFT 모순 문장 정정 | ✅ 완료 (8.4절 6항 결론으로 통일) | 9절 |
| **C11** | "+5.9%p 유의하지 않음" 문장 정정 | ✅ 완료 (부호 반전 결과로 대체) | 9절 |
| **M13** | 모든 로그 인용에 C:/F:/G: 접두어 | ✅ 완료 (70건) | 본문 전체, Appendix C/F/G 서두 |
| **M13** | 잘못된 인용 범위 정정 | ✅ 완료 (**5건** 발견·정정) | 4.2, 8.1(2곳), 8.3(2곳) |
| **M14** | 대비 지표 C의 신구 정의 표기 | ✅ 완료 ("(v3 정의)" 병기) | 8.2.2, 8.3.1, 8.4, §9 |
| **M15** | 단독 생존 사례 재해석 | ✅ 완료 (rate=0 기준선 대비로 재서술) | 8.2.5절 |
| **M16** | 8.2.5절이 정상 조건만 사용함을 한계로 명시 | ✅ 완료 (C9와 통합) | 8.2.5절 한계 박스 |
| **M17** | 권고표 용도(목적1/목적2)별 상충 설명 | ✅ 완료 (표 아래 각주로 추가 — 열 추가 아님, v6 M20이 표기 정정) | 8.4절 표 아래 각주 |
| Minor | Change Log "두 차례"→"네 차례" | ✅ 완료 | Change Log 서두 |
| Minor | FFT의 P=3 100% vs P=5 42.2% 진단 | ✅ 완료 (듀티비/sinc 가설, 정성적) | 8.2.4절 |
| Minor | §9 (b) FFT 근거를 8.2.3→8.2.4로 교체 | ✅ 완료 | 9절 남은 과제 |
| Minor | 8.2.5절 자체를 체크리스트에 등재 | ✅ 완료 | 본 표(4차 리뷰 섹션 자체가 등재) |

**4차 리뷰 미이행 항목**: 없음. M1/M7(실측 데이터 집계)은 여전히 미이행이며 사유도
그대로다(7.3절). FFT 듀티비 가설의 정량적 검증(Minor, 남은 과제 (h))과 단독 생존 사례의
추가 검증(9절 남은 과제 (g))은 성격상 "리뷰 미이행"이 아니라 **이번 개정이 새로 규명한
현상에 대한 후속 과제**로 남긴다.

**방법론 노트 — 예상과 반대 방향 결과의 자동 점검 (C8)**

M5(드리프트 역설, 3차 리뷰)와 C8(결손 개선, 4차 리뷰)은 같은 유형의 오류였다 — **결과를
"악화"만 예상하고 데이터를 그 방향으로만 훑어, 반대 방향으로 움직인 사례를 놓쳤다.** 두
번 반복된 만큼, 이후 라운드에서도 재발할 수 있는 구조적 위험으로 보고 다음을 원칙으로
남긴다.

1. 어떤 스트레스 조건(드리프트 진폭, 결손률, 오염률 등)을 스윕하는 실험을 설계할 때는,
   **베이스라인(스트레스=0) 대비 각 방법·각 패턴의 변화 방향을 전수 비교**하는 표를
   반드시 산출한다 — 평균이나 총계만 보지 않는다.
2. "악화" 방향만 강조하는 서술을 쓰기 전에, **개선된 사례가 있는지 코드로 먼저 확인**한다
   (본문에 "~할수록 나빠진다"고 쓰기 전에 `pivot table`로 방향을 뒤집은 셀이 있는지 스캔).
3. 반대 방향 사례를 발견하면 코드 버그 가능성(C8-1의 "경로 동일성 점검"과 같은 절차)을
   먼저 배제한 뒤에만 "실재하는 현상"으로 서술한다.

### 3차 리뷰 (v3 → v4) 대응

| 항목 | 요구사항 | 조치 | 반영 위치 |
|---|---|---|---|
| **C5** | 두 세트 McNemar 대조표(최소 5쌍) | ✅ 완료 | 8.4절 2항 표, 로그 F:L166-180 |
...
### 산발적 결손 조건 추가 (v4 내 추가 반영, 리뷰 항목 아님)

| 항목 | 요구사항 | 조치 | 반영 위치 |
|---|---|---|---|
| 결손 실험 | 실무 확인된 실패 모드(검출 누락)만으로 강건성 재검증, 오검출(삽입)은 제외 | ✅ 완료 (11패턴×5rate×30회=1,650회) | 8.2.5절, Appendix G |
| 대응표본 검정 | Wilson CI 겹침이 아니라 McNemar+Holm으로 확정 | ✅ 완료 (n=1,320) | 8.2.5절, 로그 G:L178-189 |
| 전체 논조 반영 | 클린 데이터 권고가 결손 위험 조건에서도 유지되는지 재검토 | ✅ 완료 — 조건별로 다른 표준 기법을 권고하도록 8.4·9·초록 재작성 | 8.4절, 9절, 초록 |
| 한계 명시 | 알림(목적 2) 성능은 결손 조건에서 미검증임을 표기 | ✅ 완료 | 8.2.5절 말미, 9절 남은 과제 (f) |

**미이행 항목**: 결손 조건에서의 알림(오경보/미탐) 재평가, P=4 카세트지연-low rate=0.5%의
단일 생존 사례에 대한 추가 검증(9절 남은 과제 (f), (g)).

### 2차 리뷰 (v2 → v3) 대응 *(v4에서 상태 유지)*

| 항목 | 요구사항 | 조치 | 반영 위치 |
|---|---|---|---|
| **C3** | 대응표본에 맞는 검정(McNemar/paired bootstrap)으로 교체 | ✅ 완료 | 8.4절 표, 로그 C:L266-294 |
| **C3** | 필수 3쌍 검정 결과 보고 | ✅ 완료 | 8.4절 표 1~3행, 로그 C:L291-294 |
| **C3** | 다중비교(15쌍) Holm/Bonferroni 보정 | ✅ 완료 | Holm 보정, 로그 C:L271-289 |
...
| **M4** | 0.06% → 정정 + 로그 인용 | ✅ 완료 (**0.71%**) | 4.4절 인용 블록, 7.1절, 로그 C:L141-146 |
| **M4** | 정정 후 결론 유지 여부 재판단 | ✅ 완료 (유지하되 강도 하향) | 4.4절 인용 블록 말미 |
| **M5** | 드리프트 상승 사례를 표로 보고 | ✅ 완료 | **8.3.1절 신설** |
| **M5** | 진폭 스윕([0,1,3,5,10])으로 진단 | ✅ 완료 | 8.3.1절, 이미지 07, 로그 C:L370-393 |
...
| **M6** | 현행 6개 패턴의 지표 값 제시 | ✅ 완료 (0.087~1.316, 15배 차) | 8.2.2절 표, 로그 C:L409-417 |
| **M6** | 대비를 맞춘 세트로 재실험 | ✅ 완료 (270회) | **8.2.3절 신설**, 이미지 08 |
| **M6** | 순위 유지 여부 확인 | ✅ 완료 (**열세 확대**) | 8.2.3절 1항, 8.4절 2항 표 *(v6 정정 — 4항은 C14로 P=6 진단 내용으로 교체되어, 열세 확대 수치는 2항의 두 세트 대조표에 위치)* |
...
| **M8** | 실험 1~4도 로그화, 표에 로그 인용 | ✅ 완료 | 로그 C:L6-118 (4.3/6.2/7.2절 표) |
...
| **C2** | 직접 위상비교의 후보 집합 재검토 및 명시 | ✅ 완료 | 8.1절, 로그 C:L166-173 |
...
| **M2** | H~N 회귀 기울기 95% CI 보고 | ✅ 완료 (v3에서 수치 정정) | 4.4절, 7.1절, 로그 C:L137-148 |
